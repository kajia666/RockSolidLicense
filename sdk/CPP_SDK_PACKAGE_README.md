...
- `examples/vs2022_cpp_host_consumer/`: ready-to-open VS2022 host-app project skeleton with `.sln`, `.vcxproj`, `RockSolidSDK.props`, and `RockSolidSDK.local.props`
...
- Prefer editing `RockSolidSDK.local.props` for machine-local SDK path, target name, or debugger working-directory overrides instead of changing the shared `RockSolidSDK.props`.
...
- The `vs2022_cpp_host_consumer` example ships `.sln`, `.vcxproj`, `RockSolidSDK.props`, and `RockSolidSDK.local.props`, so VS2022 users can open the package and build immediately without creating a project first.
...
- If the software author prefers native Visual Studio projects, the generated integration and release packages now also emit a VS2022 `.sln/.vcxproj` template plus matching `RockSolidSDK.props` and `RockSolidSDK.local.props` sheets and a project-aware `VS2022 quickstart` markdown file that points at the extracted SDK package root and first-run steps.
