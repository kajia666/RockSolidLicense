...
- 同时统一 `launch-mainline` 现在还会直接下发一份 `production handoff` 下载：它会把部署文档、健康检查脚本、备份恢复脚本和当前主存储/运行态对应的环境骨架一起收成服务端生成的交付文本，所以真正接近上线时，不用再只在 README 里翻部署材料，主线里就能直接把这份生产化 handoff 拿走
