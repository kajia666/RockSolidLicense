...
- `/admin`：管理员控制台，可处理账号、授权、会话、绑定、设备封禁和审计日志，支持审计快捷筛选与日志行回填，并可导出管理员视角的授权运营快照包
...
- `/developer/ops`：开发者授权运营台，处理账号冻结、授权续期、点数调账、强制下线、设备解绑、设备封禁，并支持审计快捷筛选、日志行回填和 scoped ops 快照包导出
