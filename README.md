...
- 这条恢复演练现在也不再只是 handoff 材料了：统一 `Launch Mainline` 会直接检查当前 lane 最近 `14` 天内有没有恢复演练 evidence；没有的话，`Production Gate Checks` 会明确给出 `Record Recovery Drill` 动作，并把这条 evidence 当成真正的 readiness gate，而不是“文档有了就算做过”；同一套 evidence 逻辑现在也已经扩到了 `Record Backup Verification`、`Record Operations Walkthrough`、`Record Deploy Verification`、`Record Health Verification` 和 `Record Rollback Walkthrough`，所以备份校验、首发值守 walkthrough、部署确认、健康确认和回滚演练也开始真正进入统一主线闸门；同时 deploy/health/rollback 这组检查现在会优先指向新的 `cutover handoff`，不再只落回泛化 production handoff，而新的 `Record Cutover Walkthrough` 也能一次满足 grouped cutover 和 deploy/health/rollback 这整组 readiness；再往上一层，`Record Launch Day Readiness Review` 现在还能一次满足 backup/recovery/ops/cutover 这一整组 launch-day evidence；而新增的 `post-launch-sweep handoff` 和 `Record First-Wave Ops Sweep` 则把“上线后第一轮 routed ops review”也正式拉进了统一主线 gate，不再只停在 launch-day rehearsals
