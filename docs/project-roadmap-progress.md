...
- Latest backend/API slice: Launch Mainline now preserves Developer Ops route-review primary and remaining download scope across product, channel, event type, and actor type, so operators can reopen the exact scoped Ops export from the mainline page without losing the release lane.
- Latest Ops export slice: Developer Ops route-review primary, next, remaining, section, and continuation downloads now include service-generated `href` values directly in the snapshot payload, reducing frontend/download-key guessing for low-frequency launch duty handoffs.
