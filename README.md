...
- 现在后台/API 里还新增了统一的 `/api/developer/launch-mainline` 聚合入口，会把 `release / workflow / review / smoke / ops` 五段主线 gate 收成一份服务端 handoff，并提供 `json / summary / checksums / zip` 下载，方便把首发主链直接作为一个连续的运营总览来消费；同时四段 launch 工作台和发版主线自己的 `recommendedDownloads` 现在也会直接回指这份统一的 `launch-mainline summary / checksums / zip`，这样 release、launch workflow、launch review、launch smoke 以及后续 ops 巡检都能顺手把同一份总览 handoff 交给 QA、值守或发布同事；现在这条统一 handoff 还对应到了独立的 `/developer/launch-mainline` 工作台，软件作者可以直接在一个页面里查看 overall gate、stage gates、统一 action plan 和推荐下载，而不只是拿一份聚合 summary 文件
...
- 现在这份 `receipt` 也会把刷新后的 unified `stages` 一起带回来，所以 `Last Mainline Action` 会直接显示 `release / workflow / review / smoke / ops` 各阶段 gate 状态，不用再主要靠前端回头翻完整 `launchMainline` payload
...
- 现在 `Last Mainline Action` 里原来单独渲染的 follow-up step 列表，也已经进一步收成了服务端下发的 `mainlineFollowUpCards`；页面不再自己把 `receipt.mainlineActions` 和顶层 follow-up 按钮拆成两套结构，而是直接渲染 `recap cards + follow-up cards`，把这块继续收回到了后台/API 主线上
...
- 在这之上，统一主线现在还会额外下发 `mainlinePage`：summary 里会给完整的 `heroControls + sections + summaryText`，动作执行后 receipt 会补齐同一份 page 快照和 `lastActionScreen`，所以 `Launch Mainline` 页面本身也开始优先围着一份统一 page model 工作，不再继续主要依赖旧的 `currentMainlineScreen` 路径
- 进一步地，`Launch Mainline` 页面里的 workspace / download 控制现在也不再主要靠页面自己按 `action.key` 或 `item.source` 去拼 URL，而是直接执行服务端下发的 `href`；这条统一主线又少了一层前端侧分发判断
...
- 统一 `Launch Mainline` 现在还额外带上了 `productionGate`，会直接把默认管理员密码、默认 server token secret、loopback/假公网入口、单机 `sqlite` 存储形态、运行态 driver 安全性、HTTPS 入口状态和 token-key 冗余这些上线前硬条件并进主线 gate；一旦这些条件阻断或需要复查，主线会直接把 lane 路由到 `Developer Security` 或 `Developer Ops`，而不是把这层生产化检查留在主线外面
- 这层 production gate 现在还会直接识别“看起来像外部依赖、实际上还挂在本机”的假生产配置：如果 `postgres` 或 `redis` 的目标端点还停在 `localhost / 127.x / ::1` 这类 loopback 地址，统一主线会直接把它拦成 `HOLD`，避免只看 driver 已经切到外部形态、却把真实上线风险漏掉
- 现在这层 `productionGate` 也不再只是 `Stage Gates` 里的一行状态了：统一主线会直接下发 `Production Gate Checks` section，把每条生产 readiness 检查作为独立卡片和可执行 controls 一起给页面和 action receipt，所以软件作者在主线里就能直接看见“到底是哪几项阻断、该先点哪个安全/运维动作”；这块现在还会把已经就位的生产化资产一起作为 pass/review 检查带出来，比如健康检查路径和 backup/restore handoff，不再只在有问题时才有内容
- 同时统一 `launch-mainline` 现在还会直接下发一份 `production handoff` 下载：它会把部署文档、健康检查脚本、备份恢复脚本和当前主存储/运行态对应的环境骨架一起收成服务端生成的交付文本，所以真正接近上线时，不用再只在 README 里翻部署材料，主线里就能直接把这份生产化 handoff 拿走；现在还额外补了一份 `operations handoff` 下载，会把 observability、告警分级、incident response、daily checklist、shift handover、launch timeline 这套值守材料也一起收成统一交付
- 统一 `launch-mainline` 现在还额外补了一份 `cutover handoff` 下载：它会把部署脚本、健康检查资产、验证信号和回滚材料收成一份服务端生成的交付文本，让 `deploy -> verify health -> run review -> watch ops -> decide rollback` 这条上线切换路径也能直接从主线里交接出去
- 现在统一 `launch-mainline` 还额外补了一份 `recovery drill handoff` 下载：它会把备份脚本、恢复脚本、存储文档、健康验证信号和“backup -> restore -> healthcheck -> review -> rollback”的演练路径一起收成一份服务端生成的交付文本，让恢复演练不再只是知道“有脚本”，而是能顺着统一主线直接排成一套可交接动作
- 这条恢复演练现在也不再只是 handoff 材料了：统一 `Launch Mainline` 会直接检查当前 lane 最近 `14` 天内有没有恢复演练 evidence；没有的话，`Production Gate Checks` 会明确给出 `Record Recovery Drill` 动作，并把这条 evidence 当成真正的 readiness gate，而不是“文档有了就算做过”；同一套 evidence 逻辑现在也已经扩到了 `Record Backup Verification`、`Record Operations Walkthrough`、`Record Deploy Verification`、`Record Health Verification` 和 `Record Rollback Walkthrough`，所以备份校验、首发值守 walkthrough、部署确认、健康确认和回滚演练也开始真正进入统一主线闸门；同时 deploy/health/rollback 这组检查现在会优先指向新的 `cutover handoff`，不再只落回泛化 production handoff，而新的 `Record Cutover Walkthrough` 也能一次满足 grouped cutover 和 deploy/health/rollback 这整组 readiness；再往上一层，`Record Launch Day Readiness Review` 现在还能一次满足 backup/recovery/ops/cutover 这一整组 launch-day evidence；新增的 `post-launch-sweep handoff` 和 `Record First-Wave Ops Sweep` 则把“上线后第一轮 routed ops review”也正式拉进了统一主线 gate，不再只停在 launch-day rehearsals；而新的 `closeout handoff` 和 `Record Launch Closeout Review` 现在会继续把 launch-day review 和 first-wave ops sweep 一起收成真正的上线收尾闭环
...
- 现在这条 `Release Mainline Follow-up` 也已经统一接上服务端生成的 `mainline gate`，会直接给出放行状态、阻断/关注计数、推荐工作台、主动作和推荐下载；release package 摘要里的 `Release Mainline Follow-up` 也会同步打印这一节 `Launch Mainline Gate`，让 release 和 launch 侧的主线口径保持一致
