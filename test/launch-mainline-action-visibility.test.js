...
test("developer launch mainline action receipt exposes visibility checkpoints for first-wave ops sweep", async () => {
  ...
  try {
    ...
    assert.equal(
      visibility?.downloads?.launchReceiptNextFollowUp?.launchDutyRecordIndexPath,
      "artifacts/staging/VISIBILITY_ALPHA/stable/launch-duty-record-index.json"
    );
    assert.match(
      visibility?.downloads?.launchReceiptNextFollowUp?.href || "",
    ...
    assert.deepEqual(
      visibility?.stagingResultBackfill?.requiredResultKeys,
      [
    ...
    assert.equal(visibility?.stagingResultBackfill?.destinations?.developerOps?.key, "ops");
    assert.equal(visibility?.stagingResultBackfill?.destinations?.launchMainline?.key, "launch-mainline");
    ...
    assert.equal(
      latestReceipt?.receiptVisibility?.downloads?.launchReceiptNextFollowUp?.launchDutyRecordIndexPath,
      "artifacts/staging/VISIBILITY_ALPHA/stable/launch-duty-record-index.json"
    );
    assert.match(
      latestReceipt?.receiptVisibility?.downloads?.launchReceiptNextFollowUp?.href || "",
      /readinessGateRecordIndex=artifacts%2Fstaging%2FVISIBILITY_ALPHA%2Fstable%2Flaunch-duty-record-index\.json/
    );
    ...
    assert.deepEqual(
      latestReceipt?.receiptVisibility?.stagingResultBackfill?.requiredResultKeys,
      [
        "route_map_gate_result",
        "backup_restore_drill_result",
        "live_write_smoke_result",
    ...
    assert.match(
      ...
      /Download Href: .*format=launch-receipt-next-follow-up.*launchDutyRecordIndex=artifacts\/staging\/VISIBILITY_ALPHA\/stable\/launch-duty-record-index\.json/
    );
    ...
    assert.match(
      postLaunchHandoffIndex.body,
      /Contract Next Required Action: .*download=developer-ops-launch-receipt-next-follow-up\.txt.*launchDutyRecordIndex=artifacts\/staging\/VISIBILITY_ALPHA\/stable\/launch-duty-record-index\.json/
    );
    assert.match(
      postLaunchHandoffIndex.body,
      /Initial Launch Operator Headline:[\s\S]*Next: operation=.*download=developer-ops-launch-receipt-next-follow-up\.txt.*launchDutyRecordIndex=artifacts\/staging\/VISIBILITY_ALPHA\/stable\/launch-duty-record-index\.json/
    );
    assert.match(
      postLaunchHandoffIndex.body,
