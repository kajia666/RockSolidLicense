...
After the operator backfills the redacted closeout template, re-run the rehearsal check with `--closeout-input-file .\artifacts\staging-closeout-template.json` to shrink `readinessGaps` before reserving the full test window.
