...
The same inline follow-up can now also download a `launch smoke kit` summary. That kit packages the current startup bootstrap request, candidate internal accounts or entitlements, fresh direct-card / recharge-card candidates, and the first smoke-test path so a project owner can hand a concrete internal validation checklist to QA, support, or launch-duty teammates right from the project workspace.
