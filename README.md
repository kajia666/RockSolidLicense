...
- 这条恢复演练现在也不再只是 handoff 材料了：统一 `Launch Mainline` 会直接检查当前 lane 最近 `14` 天内有没有恢复演练 evidence；没有的话，`Production Gate Checks` 会明确给出 `Record Recovery Drill` 动作，并把这条 evidence 当成真正的 readiness gate，而不是“文档有了就算做过”
