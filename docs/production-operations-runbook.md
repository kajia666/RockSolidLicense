...
Before launch duty accepts the switch, open the production switch proof packet from Developer Ops, Launch Mainline, Launch Review, or Launch Smoke Kit. All four surfaces should show the same packet, and the checksum/zip packages should include `production-switch-proof-packet.txt`. Treat the lane as ready only when the packet shows `ready_for_production_switch_review`, `ready=8/8`, `blocked=0/8`, and `launch_day_watch_and_stabilization` points to `first-wave-closeout.md` with the stable-operations rehearsal reload command. Keep the shared `launch-duty-record-index.json` next to the packet for the stable-operations handoff.
...
Before a staging recovery drill, run the repository rehearsal runner from a checkout of this project. It does not back up, restore, run smoke, or modify data; it verifies that the selected OS/storage profile has the expected scripts, combines the smoke and recovery gates, prints the next commands to run on the target, and includes `environmentReadiness`, `operatorChecklist`, `operatorExecutionPlan`, `resultBackfillSummary`, `stagingAcceptanceCloseout`, `receiptVisibilitySummaries`, `evidenceReadiness`, plus `evidenceActionPlan.items` for the later Launch Mainline evidence receipts. The environment readiness block tracks the public HTTPS entrypoint, non-default secret confirmation, persistent storage profile, backup/restore drill command keys, route-map gate execution, and explicit live-write approval without printing password values. The operator checklist orders the staging handoff from environment review, route-map gate, backup/restore drill, live-write approval, live-write smoke, smoke handoff archival, Launch Mainline opening, evidence recording, and receipt-visibility verification. The operator execution plan then stitches the generated output files, route-map gate, recovery drill, live-write smoke, Launch Mainline evidence, receipt visibility review, closeout template backfill, full-test-window reservation, and production sign-off review into one machine-readable sequence. Its `readinessSummary` and `readinessGaps` call out missing handoff/closeout files, missing developer bearer token, pending closeout keys, blocked full-test-window entry, and blocked production sign-off before launch duty starts copying commands. If an operator has already backfilled the redacted closeout template, pass `--closeout-input-file` to reload only the closeout key/status summary; this narrows readiness gaps without echoing raw closeout values or modifying data. After the full repository test window passes, the same closeout input can carry redacted production sign-off condition statuses; when every sign-off condition is filled and the decision is `ready-for-production-signoff`, the rehearsal check clears the production sign-off blocker. The result backfill summary names the redacted result keys to carry forward into Launch Mainline and Developer Ops after the real staging run: route-map gate result, backup/restore drill result, live-write smoke result, launch smoke handoff, Launch Mainline evidence receipts, and receipt-visibility review. The staging acceptance closeout turns those result keys into final operator checks, adds the `operator_go_no_go` decision, and points the operator to the full repository test window once redacted staging results are backfilled. Its artifact/receipt ledger maps each closeout key to a suggested redacted archive path under `artifacts/staging/<product>/<channel>` and the Launch Mainline receipt operations that should be referenced during review. The guarded full-test-window entry keeps `npm.cmd test` blocked until staging closeout is backfilled, while the production sign-off conditions require a passed full test window, archived staging artifacts, visible Launch Mainline receipts, a passed backup/restore drill, confirmed rollback path, and recorded operator sign-off before production cutover. The evidence readiness block checks the target lane, evidence endpoint, and whether `$env:RSL_DEVELOPER_BEARER_TOKEN` exists without printing the token. Each evidence item also includes a copyable PowerShell request that posts to `/api/developer/launch-mainline/action` with `$env:RSL_DEVELOPER_BEARER_TOKEN`. Add `--handoff-file` when launch duty needs a local Markdown pack to carry the smoke command, the local `npm.cmd run launch:route-map-gate` targeted route-map and download-surface gate, recovery commands, Launch Mainline URL, Launch Review / Launch Smoke receipt-visibility summary downloads, environment readiness, operator checklist, operator execution plan, result backfill summary, staging acceptance closeout, artifact/receipt ledger, full-test-window entry, production sign-off conditions, evidence readiness, evidence recording order, and evidence request snippets into the staging/live-write step without copying passwords into notes. Add `--closeout-file` when launch duty needs a redacted JSON closeout template with pending result fields, artifact paths, receipt operations, the operator execution plan, the full-test-window gate, and production sign-off conditions ready for operator backfill.
...
npm.cmd --silent run staging:rehearsal -- --json `
  ...
  --closeout-file .\artifacts\staging-closeout-template.json
```
...
After the operator backfills the redacted closeout template, re-run the rehearsal check with `--closeout-input-file .\artifacts\staging-closeout-template.json` to shrink `readinessGaps` before reserving the full test window. After the full test window passes, backfill the template's production sign-off condition statuses and rerun the same command before production cutover.
