...
- 这条恢复演练现在也不再只是 handoff 材料了：统一 `Launch Mainline` 会直接检查当前 lane 最近 `14` 天内有没有恢复演练 evidence；没有的话，`Production Gate Checks` 会明确给出 `Record Recovery Drill` 动作，并把这条 evidence 当成真正的 readiness gate，而不是“文档有了就算做过”；同一套 evidence 逻辑现在也已经扩到了 `Record Backup Verification` 和 `Record Operations Walkthrough`，所以备份校验和首发值守 walkthrough 也开始真正进入统一主线闸门
