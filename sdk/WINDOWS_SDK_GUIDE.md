...
- The packaged SDK now also includes `examples/vs2022_cpp_host_consumer/`, which is a ready-to-open VS2022 solution, project, `RockSolidSDK.props`, and `RockSolidSDK.local.props` sheet set for software authors who prefer native Visual Studio workflows.
...
- Prefer `RockSolidSDK.local.props` for workstation-specific overrides such as SDK root, output target name, or debugger working directory, so the shared `RockSolidSDK.props` stays reusable across machines.
- The project-scoped integration and release packages now also emit a matching VS2022 `.sln/.vcxproj/.props/.local.props` handoff set plus a project-aware `VS2022 quickstart` markdown guide, so software authors can start from a ready-to-open solution instead of wiring a native project by hand.
