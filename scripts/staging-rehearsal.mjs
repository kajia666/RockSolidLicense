...
const PRODUCTION_SIGNOFF_SOURCE_STEPS = {
  full_test_window_passed: "run_full_test_window",
  staging_artifacts_archived: "archive_staging_artifacts",
  launch_mainline_receipts_visible: "verify_launch_mainline_receipts",
  launch_ops_overview_status_visible: "verify_launch_ops_overview_status",
  backup_restore_drill_passed: "review_backup_restore_drill",
  rollback_path_confirmed: "confirm_rollback_path",
...
function buildSignoffConditionBackfillCommand({
  ...
  if (includeDecision) {
    parts.push("--decision", "ready-for-production-signoff");
  }
...
function buildReceiptVisibilityBackfillCommand({
  filledCloseoutInputFile,
  key,
  readinessActionQueueFile
}) {
...
function buildStagingRehearsalExecutionSummary(result) {
  ...
  const realStagingRunFocus = result.operatorExecutionPlan?.realStagingRunFocus
    || buildRealStagingRunFocus(result);
  ...
  const fullTestSignoffFocus = result.operatorExecutionPlan?.fullTestSignoffFocus
    || buildFullTestSignoffFocus(result);
  ...
  const launchDutyCurrentAction = result.finalRehearsalPacket?.launchDutyCurrentAction
    || buildLaunchDutyCurrentActionFromResult(result, { launchDutyPacketFocus });
...
function buildRehearsalProductionSwitchProofPacket(result) {
  ...
  const storageProfileProof = buildProductionSwitchStorageProfileProof(summary.storageProfile || null);
...
function buildRehearsalLaunchEvidenceReadinessGate(result) {
  const completionHandoff = result.launchDutyCompletionHandoff || null;
  if (!completionHandoff) {
    return null;
  }

  const closeoutInput = result.closeoutInput || {};
  const archiveRoot = result.stagingRunRecordTemplate?.archiveRoot
  ...
  const recordByKey = new Map((result.stagingRunRecordTemplate?.records || []).map((item) => [item.key, item]));
  const closeoutKeys = Array.isArray(closeoutInput.requiredKeys) && closeoutInput.requiredKeys.length
    ? closeoutInput.requiredKeys
    : Object.keys(CLOSEOUT_SOURCE_STEPS);
  const signoffKeys = Array.isArray(closeoutInput.requiredSignoffKeys) && closeoutInput.requiredSignoffKeys.length
    ? closeoutInput.requiredSignoffKeys
    : Object.keys(PRODUCTION_SIGNOFF_SOURCE_STEPS);
  const filledCloseoutKeys = new Set(Array.isArray(closeoutInput.filledKeys) ? closeoutInput.filledKeys : []);
  const filledSignoffKeys = new Set(Array.isArray(closeoutInput.signoffFilledKeys) ? closeoutInput.signoffFilledKeys : []);
  const missingReceiptVisibilityKeys = new Set(
    Array.isArray(closeoutInput.missingReceiptVisibilityKeys)
      ? closeoutInput.missingReceiptVisibilityKeys
      : RECEIPT_VISIBILITY_KEYS
  );
  const completedRecordKeys = new Set(
    Array.isArray(completionHandoff.completedRecordKeys)
      ? completionHandoff.completedRecordKeys
      : []
  );
  const evidenceItems = [];
  let order = 1;

  for (const key of closeoutKeys) {
    const record = recordByKey.get(key) || {};
    evidenceItems.push(buildRehearsalLaunchEvidenceItem({
      order: order++,
      key,
      type: "closeout_evidence",
      status: filledCloseoutKeys.has(key) ? "filled" : "pending_real_evidence",
      artifactPath: record.artifactPath || null,
      receiptOperations: record.receiptOperations || []
    }));
  }

  for (const key of signoffKeys) {
    evidenceItems.push(buildRehearsalLaunchEvidenceItem({
      order: order++,
      key,
      type: "production_signoff_condition",
      status: filledSignoffKeys.has(key) ? "filled" : "pending_real_evidence",
      artifactPath: key === "full_test_window_passed"
        ? path.posix.join(archiveRoot, "full-test-output.txt")
        : path.posix.join(archiveRoot, `${key.replaceAll("_", "-")}.txt`),
      receiptOperations: []
    }));
  }
  ...
  for (const key of RECEIPT_VISIBILITY_KEYS) {
    evidenceItems.push(buildRehearsalLaunchEvidenceItem({
      ...
      status: missingReceiptVisibilityKeys.has(key) ? "pending_real_evidence" : "visible",
      artifactPath: path.posix.join(archiveRoot, `${key.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}-receipt-visibility.json`),
      receiptOperations: key === "launchMainline" ? ["record_post_launch_ops_sweep"] : []
    }));
  }
  ...
  const launchDutyDefinitions = [
    {
      ...
      artifactPath: recordByKey.get("launch_day_watch_summary")?.artifactPath || path.posix.join(archiveRoot, "launch-day-watch-summary.md"),
    ...
    },
    {
      ...
      artifactPath: completionHandoff.firstWaveCloseoutArtifactPath
        || recordByKey.get("first_wave_closeout")?.artifactPath
  ...
  for (const definition of launchDutyDefinitions) {
    evidenceItems.push(buildRehearsalLaunchEvidenceItem({
      ...
      status: completedRecordKeys.has(definition.key) ? "recorded" : "pending_real_evidence",
  ...
  const currentItem = evidenceItems.find((item) => item.key === "first_wave_closeout") || null;
  const recordedCount = completionHandoff.recordedCount ?? completedRecordKeys.size;
  const pendingCount = completionHandoff.pendingCount ?? 0;

  return {
    version: "staging-rehearsal-launch-evidence-gate/v1",
    ...
    currentCommand: null,
    currentArtifactPath: currentItem?.artifactPath || null,
    closeoutInputFile: closeoutInput.path || completionHandoff.closeoutInputFile || null,
    readinessActionQueueFile: completionHandoff.actionsFile || null,
    archiveRoot,
    evidenceCount: evidenceItems.length,
    closeoutEvidenceCount: closeoutKeys.length,
    productionSignoffEvidenceCount: signoffKeys.length,
    receiptVisibilityEvidenceCount: RECEIPT_VISIBILITY_KEYS.length,
    launchDutyEvidenceCount: launchDutyDefinitions.length,
    completedEvidenceCount,
    pendingEvidenceCount,
    readinessStatusCommand: completionHandoff.statusCommand || null,
    ...
    firstWaveCloseoutArtifact: currentItem?.artifactPath || null,
    launchDutyRecordIndexPath: completionHandoff.recordIndexFile || path.posix.join(archiveRoot, "launch-duty-record-index.json"),
    launchDutyRecordProgress: {
      recorded: recordedCount,
      pending: pendingCount,
      total: recordedCount + pendingCount,
      nextRecordKey: null
    },
    stableOperationsHandoff: {
      status: completionHandoff.status || "ready_for_stabilization_handoff",
      handoffArtifacts: Array.isArray(completionHandoff.handoffArtifacts) ? completionHandoff.handoffArtifacts : [],
      nextAction: completionHandoff.nextAction || null
    },
    progress: {
      closeout: {
        completed: closeoutKeys.filter((key) => filledCloseoutKeys.has(key)).length,
        total: closeoutKeys.length
      },
      productionSignoff: {
        completed: signoffKeys.filter((key) => filledSignoffKeys.has(key)).length,
        total: signoffKeys.length
      },
      receiptVisibility: {
        completed: RECEIPT_VISIBILITY_KEYS.filter((key) => !missingReceiptVisibilityKeys.has(key)).length,
        total: RECEIPT_VISIBILITY_KEYS.length
      },
      launchDuty: {
        completed: launchDutyDefinitions.filter((item) => completedRecordKeys.has(item.key)).length,
        total: launchDutyDefinitions.length
      }
    },
    evidenceItems,
...
function buildStableOperationsFirstResultBridge(result) {
  const firstDutyBridge = result.stableOperationsFirstDutyBridge || null;
  if (!firstDutyBridge) {
    return null;
  }
  const productCode = result.summary?.productCode || result.closeoutInput?.productCode || null;
  ...
  return {
    version: "staging-rehearsal-stable-operations-first-result-bridge/v1",
    ...
    firstDutyBridgeStatus: firstDutyBridge.status || null,
    recordIndexFile: firstDutyBridge.recordIndexFile || null,
    handoffArtifacts: Array.isArray(firstDutyBridge.handoffArtifacts) ? firstDutyBridge.handoffArtifacts : [],
...
function buildStableOperationsRolloutWideningBridge(result) {
  const firstResultBridge = result.stableOperationsFirstResultBridge || null;
  if (!firstResultBridge) {
    return null;
  }
  const productCode = result.summary?.productCode || result.closeoutInput?.productCode || null;
  const channel = result.summary?.channel || result.closeoutInput?.channel || null;
  ...
  return {
    ...
    firstResultBridgeStatus: firstResultBridge.status || null,
    recordIndexFile: firstResultBridge.recordIndexFile || null,
    handoffArtifacts: Array.isArray(firstResultBridge.handoffArtifacts) ? firstResultBridge.handoffArtifacts : [],
    ...
    firstOperatingResultHandoff: firstResultBridge.firstOperatingResultHandoff || buildLaunchMainlineDownload(
      baseUrl,
      productCode,
      channel,
      "first_operating_result_handoff_execution",
    ...
    firstOperatingResultReview: firstResultBridge.firstOperatingResultReview || buildLaunchMainlineDownload(
      baseUrl,
      productCode,
      channel,
      "first_operating_result_review_execution",
    ...
    overviewStatus: firstResultBridge.overviewStatus || buildOpsDownload(
      baseUrl,
      productCode,
      channel,
      "ops_launch_operations_overview_status",
...
function buildBackupRestoreDrillExecutionEntry({
  backupRestoreBackfilled = false,
  commands = {},
  resultCaptureEntry = {},
  packetFile = null,
  ...
  const commandDefinitions = [
    ["run_app_backup", "appBackup"],
    ["run_postgres_backup", "postgresBackup"],
    ["run_postgres_restore_dry_run", "postgresRestoreDryRun"],
    ["run_restore_healthcheck", "healthcheck"]
  ];
  const commandSequence = commandDefinitions.map(([key, commandKey]) => ({
    key,
    commandKey,
    status: commands[commandKey] ? "operator_execute" : "not_available",
    command: commands[commandKey] || null
  }));
  const firstExecutableCommand = commandSequence.find((item) => item.status === "operator_execute") || null;
  const target = resultCaptureEntry.resultBackfillTarget || {};
  const reloadCommand = target.reloadCommand || closeoutReloadCommand || null;
  const {
    reloadCommand: _omitReloadCommand,
    ...closeoutBackfillTarget
  } = target;
  return {
    mode: "backup-restore-drill-execution-entry",
    status: backupRestoreBackfilled ? "ready_for_closeout_reload" : "awaiting_backup_restore_drill",
    willModifyData: false,
    currentActionKey: backupRestoreBackfilled
      ? "reload_closeout_input"
      : firstExecutableCommand?.key || resultCaptureEntry.currentActionKey || "run_backup_restore_drill",
    currentCommand: backupRestoreBackfilled
      ? reloadCommand
      : firstExecutableCommand?.command || resultCaptureEntry.currentCommand || null,
    packetFile,
    artifactPath,
    commandSequence,
    receiptQueue: resultCaptureEntry.receiptTargets || [],
    closeoutBackfillTarget,
    closeoutReload: {
      status: backupRestoreBackfilled ? "ready" : "blocked_until_backup_restore_backfill",
...
function buildStagingBackupRestoreDrillPacket(result) {
  ...
  const goLiveExecutionEntry = result.operatorExecutionPlan?.goLiveExecutionEntry
    ...
    || runRecordIndex.goLiveExecutionEntry
    || buildGoLiveExecutionEntryFromResult(result);
  const resultCaptureEntry = buildBackupRestoreResultCaptureEntry({
  ...
  const executionEntry = buildBackupRestoreDrillExecutionEntry({
    backupRestoreBackfilled,
    commands,
    resultCaptureEntry,
    packetFile,
...
function buildStagingProductionSignoffPacket(result) {
  ...
  const closeoutInputPath = bindingFiles.get("filled_closeout_input")?.path
    || runRecordIndex.closeoutProgress?.closeoutInputPath
    || path.posix.join(archiveRoot, "filled-closeout-input.json");
  const readinessActionQueueFile = bindingFiles.get("readiness_action_queue")?.path
    || result.readinessActionQueueFile?.path
    || path.posix.join(archiveRoot, "readiness-action-queue.md");
  ...
  const productionSignoffEvidenceCaptureEntries = productionSignoff.evidenceCaptureEntries
    || (closeoutInput ? buildProductionSignoffEvidenceCaptureEntries({
      closeout,
      closeoutInput,
      missingSignoffKeys,
      missingReceiptVisibilityKeys,
      productionSignoffPacketFile: packetFile,
      closeoutInputPath: closeoutInput?.path || closeoutInputPath,
      reloadCommand: closeoutReload,
      fullTestWindowCommand,
      readinessActionQueueFile,
...
function buildProductionSignoffEvidenceCaptureEntries({
  closeout = {},
  closeoutInput = null,
  missingSignoffKeys = [],
  missingReceiptVisibilityKeys = [],
  ...
  const readyForFullTestWindow = closeoutInput?.readyForFullTestWindow === true;
  const signoffTargets = buildSignoffEvidenceTargets(closeout, missingSignoffKeys);
  const receiptVisibilityTargets = buildReceiptVisibilityEvidenceTargets(missingReceiptVisibilityKeys);
  const signoffEntries = signoffTargets.map((target) => {
    const filled = target.status === "filled";
    const status = filled ? "filled" : readyForFullTestWindow ? "pending_operator_evidence" : "blocked_until_full_test_window";
    const currentActionKey = filled && canSignoff
      ? "archive_production_signoff"
      : PRODUCTION_SIGNOFF_SOURCE_STEPS[target.key] || "backfill_production_signoff";
    return {
      mode: "production-signoff-evidence-capture-entry",
      key: target.key,
      category: "signoff_condition",
      status,
      willModifyData: false,
      currentActionKey,
      currentCommand: !filled && target.key === "full_test_window_passed" && readyForFullTestWindow
        ? fullTestWindowCommand
        : null,
      resultBackfillTarget: {
        key: target.key,
        ...
        backfillCommand: filled
          ? null
          : buildSignoffConditionBackfillCommand({
            filledCloseoutInputFile: closeoutInputPath,
            key: target.key,
            includeDecision: target.key === "full_test_window_passed",
            readinessActionQueueFile
          }),
      ...
      nextAction: filled
        ? "Archive production sign-off packet and continue launch-day watch."
        : target.key === "full_test_window_passed"
          ? `Run ${fullTestWindowCommand}, capture the redacted summary, backfill ${target.key}, then reload closeout input.`
          : `Backfill ${target.key} in production sign-off, then reload closeout input.`
    };
  });
  const receiptEntries = receiptVisibilityTargets.map((target) => {
    const visible = target.status === "visible";
    const status = visible ? "visible" : readyForFullTestWindow ? "pending_visibility_review" : "blocked_until_full_test_window";
    return {
      mode: "production-signoff-evidence-capture-entry",
      key: target.key,
      category: "receipt_visibility",
      status,
      willModifyData: false,
      currentActionKey: visible && canSignoff ? "archive_production_signoff" : "verify_receipt_visibility",
      currentCommand: null,
      resultBackfillTarget: {
        key: target.key,
        status,
        ...
        reloadCommand,
        expectedEvidence: target.expectedEvidence || null,
        backfillCommand: visible
          ? null
          : buildReceiptVisibilityBackfillCommand({
            filledCloseoutInputFile: closeoutInputPath,
            key: target.key,
            readinessActionQueueFile
          }),
        ...
        visibilityDownload: receiptVisibilitySummaries?.[target.key] || null
      },
      nextAction: visible
        ? "Archive production sign-off packet and continue launch-day watch."
        : `Confirm ${target.key} receipt visibility, backfill production sign-off, then reload closeout input.`
    };
  });
...
function buildProductionSignoffEvidenceExecutionEntry(entries = [], { canSignoff = false } = {}) {
  ...
  const evidenceQueue = evidenceEntries.map((entry) => {
    ...
    return {
      ...
      closeoutInputPath: target.closeoutInputPath || null,
      expectedEvidence: target.expectedEvidence || null,
      backfillCommand: target.backfillCommand || null,
      statusCommand: target.statusCommand || null,
  ...
  return {
    ...
    currentBackfillCommand: pendingEntry?.resultBackfillTarget?.backfillCommand || null,
    statusCommand: pendingEntry?.resultBackfillTarget?.statusCommand
      || firstTarget.statusCommand
      || null,
    evidenceQueue,
...
function buildProductionSignoffReadiness(result) {
  const closeout = result.stagingAcceptanceCloseout || {};
  const closeoutInput = result.closeoutInput || null;
  const archiveRoot = closeout.artifactReceiptLedger?.archiveRoot
    || result.stagingRunRecordTemplate?.archiveRoot
    || result.finalRehearsalPacket?.archiveRoot
    || "artifacts/staging/product/stable";
  const productionSignoffPacketFile = result.productionSignoffPacketFile?.path
    || path.posix.join(archiveRoot, "staging-production-signoff-packet.json");
  const closeoutInputPath = closeoutInput?.path || path.posix.join(archiveRoot, "filled-closeout-input.json");
  const readinessActionQueueFile = result.readinessActionQueueFile?.path
    || path.posix.join(archiveRoot, "readiness-action-queue.md");
  const reloadCommand = result.closeoutBackfillGuide?.closeoutInputReload?.command || "npm.cmd run staging:rehearsal -- --closeout-input-file <filled-closeout.json>";
  const captureReloadCommand = closeoutInput?.path
    ? `npm.cmd run staging:rehearsal -- --closeout-input-file ${closeoutInput.path}`
    : reloadCommand;
  const fullTestWindowCommand = closeout.fullTestWindowEntry?.command || "npm.cmd test";
  const requiredSignoffKeys = (closeout.productionSignoffConditions?.conditions || [])
    .map((item) => item.key)
    .filter(Boolean);
  ...
  const signoffEvidenceTargets = buildSignoffEvidenceTargets(closeout, missingSignoffKeys);
  const receiptVisibilityEvidenceTargets = buildReceiptVisibilityEvidenceTargets(missingReceiptVisibilityKeys);
  return {
    ...
    missingSignoffKeys,
    missingReceiptVisibilityKeys,
    signoffEvidenceTargets,
    receiptVisibilityEvidenceTargets,
    ...(closeoutInput ? {
      evidenceCaptureEntries: buildProductionSignoffEvidenceCaptureEntries({
        closeout,
        closeoutInput,
        missingSignoffKeys,
        missingReceiptVisibilityKeys,
        productionSignoffPacketFile,
        closeoutInputPath,
        reloadCommand: captureReloadCommand,
        fullTestWindowCommand,
        readinessActionQueueFile,
        receiptVisibilitySummaries: result.nextCommands?.receiptVisibilitySummaries || null,
        canSignoff
...
function buildProductionSignoffExecutionEntry({
  ...
  const currentSignoffKey = missingSignoffKeys[0] || null;
  const currentReceiptVisibilityKey = missingReceiptVisibilityKeys[0] || null;
  const currentSignoffBackfillCommand = !canSignoff && canRunFullTestWindow && currentSignoffKey
    ? buildSignoffConditionBackfillCommand({
      filledCloseoutInputFile: closeoutInputPath,
      key: currentSignoffKey,
      includeDecision: currentSignoffKey === "full_test_window_passed",
      readinessActionQueueFile
    })
    : null;
  const currentReceiptVisibilityBackfillCommand = !canSignoff && canRunFullTestWindow && currentReceiptVisibilityKey
    ? buildReceiptVisibilityBackfillCommand({
      filledCloseoutInputFile: closeoutInputPath,
      key: currentReceiptVisibilityKey,
      readinessActionQueueFile
    })
    : null;
  ...
  return {
    ...
    signoffBackfill: {
      ...
      missingReceiptVisibilityKeys,
      currentSignoffKey,
      currentReceiptVisibilityKey,
      currentBackfillCommand: currentSignoffBackfillCommand || currentReceiptVisibilityBackfillCommand || null,
      currentSignoffBackfillCommand,
      currentReceiptVisibilityBackfillCommand,
      statusCommand
    },
    launchDayWatch: {
...
function buildFullTestSignoffFocus(result, { outputFiles = [] } = {}) {
  ...
  const currentEvidenceEntry = Array.isArray(productionSignoffEvidenceExecutionEntry.evidenceQueue)
    ? productionSignoffEvidenceExecutionEntry.evidenceQueue.find(
      (entry) => entry.key === productionSignoffEvidenceExecutionEntry.currentEvidenceKey
    ) || productionSignoffEvidenceExecutionEntry.evidenceQueue[0] || null
    : null;
  if (canRunFullTestWindow && !canSignoffProduction) {
    currentAction.followUpBackfillCommand = currentEvidenceEntry?.backfillCommand
      || buildSignoffConditionBackfillCommand({
        filledCloseoutInputFile,
    ...
    currentAction.statusCommand = currentEvidenceEntry?.statusCommand
      || productionSignoffEvidenceExecutionEntry.statusCommand
      || buildReadinessStatusCommand({
        filledCloseoutInputFile,
        readinessActionQueueFile
      });
  }
  return {
    ...
    commands: {
      ...
      signoffBackfill: currentAction.followUpBackfillCommand || null,
      readinessStatus: currentAction.statusCommand || null
    },
    paths: {
      readinessReviewPacketFile,
      productionSignoffPacketFile,
...
function buildLaunchDutyPacketFocus(result, { closeoutBackfillFocus = null } = {}) {
  ...
  const currentWatchReceiptQueue = readyForCutoverWatch && Array.isArray(watchEvidenceExecutionEntry?.receiptQueue)
    ...
    : [];
  const stabilizationHandoffExecutionEntry = readyForCutoverWatch
    ? result.stabilizationHandoffPlan?.handoffExecutionEntry || archiveIndex.stabilizationHandoff?.handoffExecutionEntry || null
    : null;
  const firstWaveCloseoutExecutionEntry = readyForCutoverWatch
    ? result.stabilizationHandoffPlan?.firstWaveCloseoutExecutionEntry || archiveIndex.stabilizationHandoff?.firstWaveCloseoutExecutionEntry || null
    : null;
  ...
  return {
    ...
    stabilizationHandoffExecutionEntry,
    firstWaveCloseoutExecutionEntry,
...
function buildGoLiveExecutionEntry({
  ...
  const fullTestCommand = fullTestSignoffFocus?.commands?.fullTestWindow
    || closeoutBackfillFocus?.fullTestWindow?.command
    || realStagingRunFocus?.fullTestEntry?.command
    || launchDutyPacketFocus?.commands?.fullTestWindow
    || "npm.cmd test";
  const closeoutReloadCommand = closeoutBackfillFocus?.reloadCommand
    || realStagingRunFocus?.commands?.closeoutReload
    || launchDutyPacketFocus?.commands?.closeoutReload
    || fullTestSignoffFocus?.commands?.closeoutReload
    || null;
  const signoffBackfillCommand = fullTestSignoffFocus?.currentAction?.followUpBackfillCommand
    || fullTestSignoffFocus?.commands?.signoffBackfill
    || null;
  const readinessStatusCommand = fullTestSignoffFocus?.currentAction?.statusCommand
    || fullTestSignoffFocus?.commands?.readinessStatus
    || realStagingRunFocus?.commands?.readinessStatus
    || null;
  ...
  const paths = {
    filledCloseoutInputFile: fullTestSignoffFocus?.paths?.filledCloseoutInputFile
      ...
      || realStagingRunFocus?.paths?.filledCloseoutInputFile
      || null,
    closeoutReloadPacketFile: launchDutyPacketFocus?.controlPaths?.closeoutReloadPacket
      || closeoutBackfillFocus?.paths?.closeoutReloadPacketFile
      || realStagingRunFocus?.paths?.closeoutReloadPacketFile
      || null,
    readinessReviewPacketFile: fullTestSignoffFocus?.paths?.readinessReviewPacketFile
      || launchDutyPacketFocus?.controlPaths?.readinessReviewPacket
      || null,
    productionSignoffPacketFile: fullTestSignoffFocus?.paths?.productionSignoffPacketFile
      || launchDutyPacketFocus?.controlPaths?.productionSignoffPacket
      || null,
    launchDutyArchiveIndexFile: launchDutyPacketFocus?.archiveIndexPath
      || realStagingRunFocus?.paths?.launchDutyArchiveIndexFile
      || null,
    artifactArchiveRoot: realStagingRunFocus?.paths?.artifactArchiveRoot
      || launchDutyPacketFocus?.archiveRoot
      || null
  };
  const blockerSummary = {
    missingCloseoutKeys: uniq([
      ...(fullTestSignoffFocus?.missingCloseoutKeys || []),
      ...(closeoutBackfillFocus?.missingBackfillKeys || []),
      ...(closeoutBackfillFocus?.fullTestWindow?.missingCloseoutKeys || []),
      ...(realStagingRunFocus?.fullTestEntry?.missingCloseoutKeys || [])
    ]),
    missingSignoffKeys: uniq(fullTestSignoffFocus?.missingSignoffKeys || closeoutBackfillFocus?.productionSignoff?.missingSignoffKeys || []),
    missingReceiptVisibilityKeys: uniq(fullTestSignoffFocus?.missingReceiptVisibilityKeys || closeoutBackfillFocus?.productionSignoff?.missingReceiptVisibilityKeys || []),
    missingSecretEnv: uniq(realStagingRunFocus?.missingSecretEnv || []),
    missingOutputFiles: uniq(realStagingRunFocus?.missingOutputFiles || [])
  };
  const canRunFullTestWindow = fullTestSignoffFocus?.canRunFullTestWindow === true
    || closeoutBackfillFocus?.fullTestWindow?.canRun === true
    || realStagingRunFocus?.fullTestEntry?.canRun === true;
  const canSignoffProduction = fullTestSignoffFocus?.canSignoffProduction === true
    || closeoutBackfillFocus?.productionSignoff?.canSignoff === true;
  let currentPhase = "full_test_window_entry";
  ...
  if (launchDutyCompletionHandoff) {
    ...
    status = fullTestSignoffFocus?.status || launchDutyCurrentAction?.status || "ready_for_launch_day_watch";
    currentCommand = launchDutyCurrentAction?.command || null;
    nextAction = launchDutyPacketFocus?.nextAction || fullTestSignoffFocus?.nextAction || launchDutyCurrentAction?.nextAction || null;
  } else if (canRunFullTestWindow) {
    currentPhase = "full_test_window_entry";
    sourceFocus = "fullTestSignoffFocus";
    currentActionKey = "run_full_test_window";
    status = "ready_for_full_test_window";
    currentCommand = fullTestCommand;
    nextAction = fullTestSignoffFocus?.nextAction
      || "Run the full test window, then backfill production sign-off evidence and reload closeout input.";
  } else if (
    realStagingRunFocus?.currentAction?.key === "run_staging_dry_run"
    && realStagingRunFocus?.canRunDryRun === true
  ) {
    currentPhase = "real_staging_inputs";
    sourceFocus = "realStagingRunFocus";
    currentActionKey = "run_staging_dry_run";
    status = realStagingRunFocus.status || "ready_for_real_staging_rehearsal";
    currentCommand = realStagingRunFocus.currentAction.command || realStagingRunFocus.commands?.stagingDryRun || null;
    nextAction = realStagingRunFocus.currentAction.nextAction || realStagingRunFocus.nextAction || null;
  } else if (!closeoutBackfillFocus && realStagingRunFocus) {
    const current = realStagingRunFocus.currentAction || {};
    currentPhase = "real_staging_inputs";
    sourceFocus = "realStagingRunFocus";
    currentActionKey = current.key || "load_staging_profile";
    status = current.status || realStagingRunFocus.status || "blocked";
    currentCommand = current.command || realStagingRunFocus.commands?.stagingDryRun || null;
  ...
  return {
    ...
    canSignoffProduction,
    commands: {
      stagingDryRun: realStagingRunFocus?.commands?.stagingDryRun || null,
      closeoutReload: closeoutReloadCommand,
      fullTestWindow: fullTestCommand,
      signoffBackfill: signoffBackfillCommand,
      readinessStatus: readinessStatusCommand
    },
    paths,
    packetFocus: {
      currentPacketKey: currentPacket.key || null,
      currentPacketStatus: currentPacket.status || null,
      currentPacketPath: currentPacket.path || null,
      archiveIndexPath: launchDutyPacketFocus?.archiveIndexPath || null
    },
    blockerSummary,
...
function buildLaunchDutyCurrentAction({
  ...
  if (fullTestSignoffFocus?.canSignoffProduction === true) {
    ...
    const followUpStabilizationSourceQueue = Array.isArray(stabilizationHandoffExecutionEntry?.sourceRecordQueue)
      ? stabilizationHandoffExecutionEntry.sourceRecordQueue
      : [];
    const followUpFirstWaveCloseoutTarget = firstWaveCloseoutExecutionEntry?.closeoutTarget
      || stabilizationHandoffExecutionEntry?.firstWaveCloseoutTarget
      || null;
    ...
    const productionSignoffPacketPath = current.packetPath
      || target.path
      || launchDutyPacketFocus?.controlPaths?.productionSignoffPacket
      || archiveContext?.productionSignoffPacketPath
      || (archiveRoot ? path.posix.join(archiveRoot, "staging-production-signoff-packet.json") : null);
    const launchDayWatchSummaryPath = watch.path
      || (archiveRoot ? path.posix.join(archiveRoot, "launch-day-watch-summary.md") : null);
    const stabilizationPath = stabilization.path
      || (archiveRoot ? path.posix.join(archiveRoot, "stabilization-owner-handoff.md") : null);
    const evidenceInputs = [
      {
        key: "production_signoff_packet",
        kind: "packet",
        status: target.status || "archive_before_cutover",
        path: productionSignoffPacketPath
      },
      {
        ...
        path: launchDayWatchSummaryPath
      },
      {
  ...
  if (fullTestSignoffFocus?.canRunFullTestWindow === true) {
    ...
    return attachLaunchDutyArchiveTrace({
      ...
      command: current.command || fullTestSignoffFocus.commands?.fullTestWindow || null,
      followUpBackfillCommand: current.followUpBackfillCommand || fullTestSignoffFocus.commands?.signoffBackfill || null,
      statusCommand: current.statusCommand || fullTestSignoffFocus.commands?.readinessStatus || null,
...
function buildLaunchDutyCurrentActionFromResult(result, { outputFiles = [], launchDutyPacketFocus = null } = {}) {
  ...
  const packetFocus = launchDutyPacketFocus
    ...
    || (result.stagingLaunchDutyArchiveIndex ? buildLaunchDutyPacketFocus(result, { closeoutBackfillFocus }) : null);
  const archiveContext = buildLaunchDutyArchiveTraceContext(result, { outputFiles, launchDutyPacketFocus: packetFocus });
...
function buildLaunchDutyArchiveTraceContext(result, { outputFiles = [], launchDutyPacketFocus = null } = {}) {
  const bindingFiles = result.stagingEnvironmentBinding?.recommendedOutputFiles || [];
  const runRecordIndex = result.stagingRehearsalRunRecordIndex || null;
  const archiveIndex = result.stagingLaunchDutyArchiveIndex || null;
  const archiveRoot = launchDutyPacketFocus?.archiveRoot
    || archiveIndex?.archiveRoot
    || runRecordIndex?.archiveRoot
    || result.finalRehearsalPacket?.archiveRoot
    || result.stagingRunRecordTemplate?.archiveRoot
    || result.stagingAcceptanceCloseout?.artifactReceiptLedger?.archiveRoot
    || null;
  const pathFor = (key, fallbackName) => keyedPath(outputFiles, key)
    || keyedPath(bindingFiles, key)
    || (archiveRoot && fallbackName ? path.posix.join(archiveRoot, fallbackName) : null);
  return {
    archiveRoot,
    ...
    launchDutyArchiveStatus: archiveIndex?.status || launchDutyPacketFocus?.status || null,
    recordGroups: runRecordIndex?.recordGroups || []
  };
}
...
function launchDutyRecordGroupKeyForStage(stage) {
  ...
  if (stage === "full_test_signoff") {
    return "production_signoff";
  }
  return "pre_full_test_closeout";
}

function attachLaunchDutyArchiveTrace(action, archiveContext = null) {
  if (!action || !archiveContext) {
    return action;
  }
  const currentRecordGroupKey = launchDutyRecordGroupKeyForStage(action.stage);
  const recordGroup = findKeyedItem(archiveContext.recordGroups, currentRecordGroupKey) || {};
  const archiveTrace = {
    ...
    runRecordIndexStatus: archiveContext.runRecordIndexStatus || null,
    launchDutyArchiveStatus: archiveContext.launchDutyArchiveStatus || null
  };
  const recordUpdates = (action.evidenceInputs || []).map((item) => ({
    key: item.key || null,
    groupKey: currentRecordGroupKey,
    status: item.status || null,
    kind: item.kind || null,
    path: item.path || null,
    command: item.command || null
...
function buildFirstWaveCloseoutExecutionEntry({
  canStartStabilizationHandoff = false,
  firstWaveCloseoutCaptureEntry = null,
  firstWaveCloseoutGate = {}
} = {}) {
  const captureEntry = firstWaveCloseoutCaptureEntry || {};
  const target = captureEntry.resultBackfillTarget || {};
  const sourceRecords = Array.isArray(target.sourceRecords) ? target.sourceRecords : firstWaveCloseoutGate.sourceRecords || [];
  const receiptTargets = Array.isArray(captureEntry.receiptTargets) ? captureEntry.receiptTargets : [];
  return {
    mode: "first-wave-closeout-execution-entry",
    status: canStartStabilizationHandoff ? "awaiting_first_wave_closeout" : "blocked_until_stabilization_owner_handoff",
    willModifyData: false,
    currentActionKey: captureEntry.currentActionKey || (canStartStabilizationHandoff ? "close_first_wave" : "verify_cutover_watch_records"),
    currentCommand: captureEntry.currentCommand || null,
    closeoutTarget: {
      key: target.key || captureEntry.key || "first_wave_closeout",
      status: target.status || captureEntry.status || (canStartStabilizationHandoff ? "operator_closeout" : "blocked_until_stabilization_owner_handoff"),
      path: target.artifactPath || firstWaveCloseoutGate.firstWaveCloseoutPath || null,
      ownerHandoffPath: target.ownerHandoffPath || firstWaveCloseoutGate.ownerHandoffPath || null,
      requiredSourceRecordKeys: target.requiredSourceRecordKeys || firstWaveCloseoutGate.requiredSourceRecordKeys || [],
      expectedEvidence: target.expectedEvidence || firstWaveCloseoutGate.expectedEvidence || null
    },
    sourceRecordQueue: sourceRecords.map((item) => ({
      key: item[0] || null,
      status: item[1] || null,
      path: item[2] || null
    })),
    receiptQueue: receiptTargets.map((item) => ({
      key: target.key || captureEntry.key || "first_wave_closeout",
      operation: item.operation || null,
      status: item.status || null,
      artifactPath: item.artifactPath || target.artifactPath || firstWaveCloseoutGate.firstWaveCloseoutPath || null
    })),
    nextAction: captureEntry.nextAction || firstWaveCloseoutGate.nextAction || null
  };
}
...
function buildStabilizationHandoffPlan(result) {
  ...
  const firstWaveCloseoutSourceRecords = (firstWaveCloseoutWindow.sourceRecordKeys || [])
    ...
    .filter(Boolean);
  const firstWaveCloseoutGate = {
    status: canStartStabilizationHandoff ? "ready_for_first_wave_closeout" : "blocked_until_stabilization_handoff",
    currentHandoffTargetKey: currentHandoffTarget?.key || null,
    ownerHandoffPath: sourceRecordByKey.get("stabilization_owner_handoff")?.path || handoffWindows[0]?.path || null,
    firstWaveCloseoutPath: firstWaveCloseoutWindow.path || path.posix.join(archiveRoot, "first-wave-closeout.md"),
    requiredSourceRecordKeys: firstWaveCloseoutWindow.sourceRecordKeys || [],
    sourceRecords: firstWaveCloseoutSourceRecords.map((item) => [item.key, item.status, item.path]),
    receiptOperations: Array.isArray(firstWaveCloseoutWindow.receiptOperations) ? firstWaveCloseoutWindow.receiptOperations : [],
    expectedEvidence: firstWaveCloseoutWindow.expectedEvidence || null,
    nextAction: canStartStabilizationHandoff
  ...
  const firstWaveCloseoutExecutionEntry = buildFirstWaveCloseoutExecutionEntry({
    canStartStabilizationHandoff,
...
function buildFinalRehearsalPacket(result) {
  ...
  const currentWatchReceiptQueue = Array.isArray(watchEvidenceExecutionEntry?.receiptQueue)
    ? watchEvidenceExecutionEntry.receiptQueue.filter((item) => item.key === (currentWatchEvidence?.key || watchEvidenceExecutionEntry.currentEvidenceKey))
    : [];
  const stabilizationHandoffExecutionEntry = stabilizationHandoffPlan.handoffExecutionEntry || null;
  const firstWaveCloseoutExecutionEntry = stabilizationHandoffPlan.firstWaveCloseoutExecutionEntry || null;
...
function buildStagingOperatorExecutionPlan(result) {
  ...
  const launchDutyArchiveContext = buildLaunchDutyArchiveTraceContext(result, { outputFiles, launchDutyPacketFocus });
  const launchDutyCurrentAction = buildLaunchDutyCurrentAction({
    realStagingRunFocus,
    closeoutBackfillFocus,
    fullTestSignoffFocus,
...
function buildResult(options) {
  ...
  const launchEvidenceReadinessGate = gatesPassed ? buildRehearsalLaunchEvidenceReadinessGate(resultWithOperatorExecutionPlan) : null;
  const resultWithLaunchEvidenceReadinessGate = {
  ...
  const stableOperationsFirstDutyBridge = gatesPassed
    ...
    : null;
  const stableOperationsFirstResultBridge = gatesPassed
    ? buildStableOperationsFirstResultBridge({
      ...resultWithLaunchExecutionPhasePlan,
      operatorQueueCheckpoint,
      ...(stableOperationsReadbackBridge ? { stableOperationsReadbackBridge } : {}),
    ...
    : null;
  const stableOperationsRolloutWideningBridge = gatesPassed
    ? buildStableOperationsRolloutWideningBridge({
      ...resultWithLaunchExecutionPhasePlan,
      operatorQueueCheckpoint,
      ...(stableOperationsReadbackBridge ? { stableOperationsReadbackBridge } : {}),
      ...(stableOperationsFirstDutyBridge ? { stableOperationsFirstDutyBridge } : {}),
      ...(stableOperationsFirstResultBridge ? { stableOperationsFirstResultBridge } : {})
    })
    : null;
  return {
    ...resultWithLaunchExecutionPhasePlan,
    ...
    ...(stableOperationsFirstDutyBridge ? { stableOperationsFirstDutyBridge } : {}),
    ...(stableOperationsFirstResultBridge ? { stableOperationsFirstResultBridge } : {}),
    ...(stableOperationsRolloutWideningBridge ? { stableOperationsRolloutWideningBridge } : {}),
...
function renderStagingRehearsalExecutionSummary(summary) {
  ...
  const goLiveExecutionEntry = focus.goLiveExecutionEntry || {};
  ...
  const lines = [
    ...
    `- Execution launch-duty archive trace: ${renderLaunchDutyArchiveTrace(launchDutyCurrentAction.archiveTrace)}`,
    `- Execution launch-duty record updates: ${renderLaunchDutyRecordUpdates(launchDutyCurrentAction.recordUpdates)}`,
...
function renderStableOperationsFirstResultBridge(bridge) {
  if (!bridge) {
    return "- Not available";
  }
  ...
  return [
    `- Stable first-result bridge: \`${bridge.status || "-"}\``,
    `- Stable first-result current: \`${bridge.currentActionKey || "-"}\` -> \`${bridge.nextActionKey || "-"}\``,
    `- Stable first-result first-duty status: \`${bridge.firstDutyBridgeStatus || "-"}\``,
    `- Stable first-result handoff: \`${handoff.fileName || "-"}\` \`${handoff.format || "-"}\` -> \`${handoff.href || "-"}\``,
    `- Stable first-result receipt readback: \`${receiptReadback.fileName || "-"}\` \`${receiptReadback.format || "-"}\` -> \`${receiptReadback.href || "-"}\``,
    `- Stable first-result review: \`${review.fileName || "-"}\` \`${review.format || "-"}\` -> \`${review.href || "-"}\``,
    `- Stable first-result overview status: \`${overview.fileName || "-"}\` \`${overview.format || "-"}\` -> \`${overview.href || "-"}\``,
    `- Stable first-result artifacts: ${(bridge.handoffArtifacts || []).join("; ") || "-"}`,
    `- Stable first-result next action: ${bridge.nextAction || "-"}`
  ].join("\n");
}

function renderStableOperationsRolloutWideningBridge(bridge) {
  if (!bridge) {
    return "- Not available";
  }
  ...
  const overview = bridge.overviewStatus || {};
  return [
    `- Stable rollout bridge: \`${bridge.status || "-"}\``,
    `- Stable rollout current: \`${bridge.currentActionKey || "-"}\` -> \`${bridge.nextActionKey || "-"}\``,
    `- Stable rollout first-result status: \`${bridge.firstResultBridgeStatus || "-"}\``,
    `- Stable rollout widening decision: \`${rollout.fileName || "-"}\` \`${rollout.format || "-"}\` -> \`${rollout.href || "-"}\``,
    `- Stable rollout first-result handoff: \`${firstHandoff.fileName || "-"}\` \`${firstHandoff.format || "-"}\` -> \`${firstHandoff.href || "-"}\``,
    `- Stable rollout first-result review: \`${firstReview.fileName || "-"}\` \`${firstReview.format || "-"}\` -> \`${firstReview.href || "-"}\``,
    `- Stable rollout next decision: \`${nextDecision.fileName || "-"}\` \`${nextDecision.format || "-"}\` -> \`${nextDecision.href || "-"}\``,
    `- Stable rollout monitoring: \`${monitoring.fileName || "-"}\` \`${monitoring.format || "-"}\` -> \`${monitoring.href || "-"}\``,
    `- Stable rollout overview status: \`${overview.fileName || "-"}\` \`${overview.format || "-"}\` -> \`${overview.href || "-"}\``,
    `- Stable rollout artifacts: ${(bridge.handoffArtifacts || []).join("; ") || "-"}`,
    `- Stable rollout next action: ${bridge.nextAction || "-"}`
  ].join("\n");
}
...
function renderLaunchEvidenceReadinessGate(gate = null) {
  if (!gate) {
    return "- Not available";
  }
  const lines = [
    `Launch evidence gate: \`${gate.status || "-"}\` (current \`${gate.currentEvidenceKey || "-"}\`, pending \`${gate.pendingEvidenceCount ?? "-"}/${gate.evidenceCount ?? "-"}\`)`,
    `Launch evidence current: \`${gate.currentEvidenceType || "-"}/${gate.currentEvidenceKey || "-"}\` -> \`${gate.currentCommand || "-"}\``,
  ...
  if (gate.stableOperationsHandoff) {
    const handoffArtifacts = Array.isArray(gate.stableOperationsHandoff.handoffArtifacts)
      ...
      : "-";
    lines.push(`Launch evidence stable handoff: \`${gate.stableOperationsHandoff.status || "-"}\` -> ${handoffArtifacts || "-"}`);
  }
  lines.push(`Launch evidence status refresh: \`${gate.readinessStatusCommand || "-"}\``);
  lines.push(`Launch evidence rehearsal reload: \`${gate.rehearsalReloadCommand || "-"}\``);
  lines.push(`Launch evidence next action: ${gate.nextAction || "-"}`);
  return lines.join("\n");
}

function writeLaunchEvidenceReadinessGatePlain(gate = null) {
  if (!gate) {
    return;
  }
...
function writeStableOperationsFirstResultBridgePlain(bridge = null) {
  if (!bridge) {
    return;
  }
  const handoff = bridge.firstOperatingResultHandoff || {};
...
function writeStableOperationsRolloutWideningBridgePlain(bridge = null) {
  if (!bridge) {
    return;
  }
  const rollout = bridge.rolloutWideningDecision || {};
...
function appendGoLiveExecutionEntry(lines, entry = {}) {
  ...
  lines.push(`- Go-live execution signoff backfill: \`${entry.commands?.signoffBackfill || "-"}\``);
  lines.push(`- Go-live execution readiness status: \`${entry.commands?.readinessStatus || "-"}\``);
  ...
  if (launchDayWatchEntry) {
    ...
    const stabilizationHandoffExecutionEntry = launchDayWatchEntry.stabilizationHandoffExecutionEntry || {};
    const firstWaveCloseoutExecutionEntry = launchDayWatchEntry.firstWaveCloseoutExecutionEntry || {};
    ...
    lines.push(`- Go-live launch-day stabilization handoff: ${stabilizationHandoffExecutionEntry.currentHandoffTarget?.key || "-"} -> ${stabilizationHandoffExecutionEntry.currentHandoffTarget?.path || "-"}`);
    lines.push(`- Go-live launch-day stabilization sources: ${renderLaunchDutyRecordUpdates(stabilizationHandoffExecutionEntry.sourceRecordQueue || [])}`);
    lines.push(`- Go-live launch-day first-wave closeout: ${firstWaveCloseoutExecutionEntry.closeoutTarget?.key || "-"} -> ${firstWaveCloseoutExecutionEntry.closeoutTarget?.path || "-"}`);
    lines.push(`- Go-live launch-day first-wave receipts: ${(firstWaveCloseoutExecutionEntry.receiptQueue || []).map((item) => `${item.key || "-"}=${item.operation || "-"}:${item.status || "-"}`).join(", ") || "-"}`);
...
function appendProductionSignoffEvidenceCaptureEntries(lines, entries = []) {
  if (!Array.isArray(entries) || !entries.length) {
    return;
  }
  lines.push(`- Production sign-off evidence capture entries: ${entries.length}`);
  for (const entry of entries) {
    const target = entry.resultBackfillTarget || {};
    lines.push(`  - ${entry.key || "-"}: ${entry.status || "-"} (${entry.category || "-"}, action=${entry.currentActionKey || "-"}) -> ${target.productionSignoffPacketFile || "-"}`);
    lines.push(`    - closeoutInput: ${target.closeoutInputPath || "-"}`);
    lines.push(`    - backfillCommand: \`${target.backfillCommand || "-"}\``);
    lines.push(`    - statusCommand: \`${target.statusCommand || "-"}\``);
    lines.push(`    - reloadCommand: \`${target.reloadCommand || "-"}\``);
    if (target.visibilityDownload) {
      lines.push(`    - visibilityDownload: ${target.visibilityDownload}`);
    }
  }
}
...
function appendFirstWaveCloseoutExecutionEntry(lines, entry, {
  entryLabel = "First-wave closeout execution entry",
  sourceLabel = "First-wave closeout execution sources",
  receiptLabel = "First-wave closeout execution receipts"
} = {}) {
  if (!entry) {
    return;
  }
  const target = entry.closeoutTarget || {};
  const sourceQueue = Array.isArray(entry.sourceRecordQueue) ? entry.sourceRecordQueue : [];
  const receiptQueue = Array.isArray(entry.receiptQueue) ? entry.receiptQueue : [];
  lines.push(`- ${entryLabel}: ${entry.status || "-"} (action=${entry.currentActionKey || "-"}, target=${target.key || "-"})`);
  lines.push(`- ${sourceLabel}: ${sourceQueue.map((item) => `${item.key || "-"}=${item.status || "-"} -> ${item.path || "-"}`).join("; ") || "-"}`);
  lines.push(`- ${receiptLabel}: ${receiptQueue.map((item) => `${item.key || "-"}=${item.operation || "-"}:${item.status || "-"}`).join(", ") || "-"}`);
  lines.push(`- ${entryLabel} next action: ${entry.nextAction || "-"}`);
}
...
function renderOperatorExecutionPlan(plan) {
  ...
  if (plan.fullTestSignoffFocus) {
    ...
    lines.push(`- Full-test signoff follow-up backfill: \`${current.followUpBackfillCommand || focus.commands?.signoffBackfill || "-"}\``);
    lines.push(`- Full-test signoff readiness status: \`${current.statusCommand || focus.commands?.readinessStatus || "-"}\``);
    ...
    lines.push(`- Full-test signoff action queue: ${focus.paths?.readinessActionQueueFile || "-"}`);
    ...
    appendProductionSignoffEvidenceCaptureEntries(lines, focus.productionSignoffEvidenceCaptureEntries || []);
...
function renderProductionSignoffReadiness(readiness) {
  ...
  appendProductionSignoffEvidenceCaptureEntries(lines, readiness.evidenceCaptureEntries || []);
  return lines.join("\n");
}
...
function renderStagingBackupRestoreDrillPacket(packet) {
  ...
  const lines = [
    ...
    `- Backup/restore execution entry: ${executionEntry.status || "-"} (action=${executionEntry.currentActionKey || "-"}, target=${executionBackfillTarget.key || "-"})`,
    `- Backup/restore execution current command: \`${executionEntry.currentCommand || "-"}\``,
    `- Backup/restore execution command sequence: ${executionCommandSequence.map((item) => `${item.key || "-"}:${item.status || "-"}`).join(", ") || "-"}`,
    `- Backup/restore execution receipts: ${executionReceiptQueue.map((item) => `${item.operation || "-"}:${item.status || "-"}`).join(", ") || "-"}`,
    `- Backup/restore execution reload: ${executionReload.status || "-"} -> \`${executionReload.command || "-"}\``,
...
function renderStagingProductionSignoffPacket(packet) {
  ...
  if (signoffExecutionEntry.status) {
    ...
    lines.push(`- Production signoff execution current backfill: \`${signoffExecutionEntry.signoffBackfill?.currentBackfillCommand || "-"}\``);
    lines.push(`- Production signoff execution readiness status: \`${signoffExecutionEntry.signoffBackfill?.statusCommand || "-"}\``);
  ...
  if (productionSignoffEvidenceExecutionEntry.status) {
    ...
    lines.push(`- Production signoff evidence current backfill: \`${productionSignoffEvidenceExecutionEntry.currentBackfillCommand || "-"}\``);
    lines.push(`- Production signoff evidence readiness status: \`${productionSignoffEvidenceExecutionEntry.statusCommand || "-"}\``);
  ...
  appendOperatorGoNoGoResultCaptureEntry(lines, packet.operatorGoNoGoResultCaptureEntry || {});
  appendGoLiveExecutionEntry(lines, packet.goLiveExecutionEntry || {});
  appendProductionSignoffEvidenceCaptureEntries(lines, packet.productionSignoffEvidenceCaptureEntries || []);
...
function renderStagingLaunchDutyArchiveIndex(index) {
  ...
  if (stabilization.firstWaveCloseoutGate?.status) {
    const gate = stabilization.firstWaveCloseoutGate;
    lines.push(`- Archive first-wave closeout gate: ${gate.status || "-"} (owner=${gate.ownerHandoffPath || "-"}, closeout=${gate.firstWaveCloseoutPath || "-"})`);
    lines.push(`- Archive first-wave closeout sources: ${(gate.sourceRecords || []).map((item) => `${item[0]}=${item[1]} -> ${item[2]}`).join("; ") || "-"}`);
  ...
  appendFirstWaveCloseoutExecutionEntry(lines, stabilization.firstWaveCloseoutExecutionEntry || null, {
    entryLabel: "Archive first-wave closeout execution entry",
    sourceLabel: "Archive first-wave closeout execution sources",
    receiptLabel: "Archive first-wave closeout execution receipts"
  });
...
function renderHandoffFile(result) {
  return [
    ...
    "",
    "## Stable Operations First Result Bridge",
    "",
    renderStableOperationsFirstResultBridge(result.stableOperationsFirstResultBridge),
    "",
    "## Stable Operations Rollout Widening Bridge",
    "",
    renderStableOperationsRolloutWideningBridge(result.stableOperationsRolloutWideningBridge),
    "",
    "## Gate Status",
    "",
    ...
    "",
    renderLaunchEvidenceReadinessGate(result.launchEvidenceReadinessGate),
    "",
...
function writeResult(result, json) {
  ...
  if (result.status === "pass") {
    ...
    writeStableOperationsFirstResultBridgePlain(result.stableOperationsFirstResultBridge);
    writeStableOperationsRolloutWideningBridgePlain(result.stableOperationsRolloutWideningBridge);
