...
- use the ops snapshot overview to surface focus accounts, sessions, and devices with severity, next-action hints, and a recommended action queue, then click back into matching filters directly
...
- `/admin` ops snapshot preview now surfaces common reasons plus focus account/session/device detail lists, each with severity and next-action hints, and also builds a recommended action queue so operators can start with the highest-priority object before drilling down further
