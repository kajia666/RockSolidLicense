...
4. Expand grouped verification toward the full pre-staging gate.
   A broader targeted batch now passes across release package, launch workflow, launch mainline actions, first-wave recommendations, Developer Ops export, launch pages, and the `launch:smoke` CLI preflight. Keep full tests deferred only briefly; the next verification step should add account/card/session authorization and deployment/security/static checks before the full pre-staging run.
...
- Latest grouped targeted verification passed 21 tests across release package, launch workflow, launch mainline actions, first-wave recommendations, Developer Ops export, launch pages, and `launch:smoke`.
- Keep full test suite deferred only until the next account/card/session and deployment/security grouped checks are run.
