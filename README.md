...
- `/admin`：管理员控制台，可处理账号、授权、会话、绑定、设备封禁和审计日志，支持审计快捷筛选、日志行回填、可点击的快照概览卡、直接可点的授权审查列表，以及从快照重点对象自动联动相关列表并高亮匹配记录、自动聚焦对应控制卡片，并在动作成功后围绕同一对象自动刷新上下文，还会在快照区显示最近一次动作是否仍处于重点关注中、风险级别是改善还是升高、是否移出了推荐处置队列、当前应当 `Close Out / Monitor / Follow Up / Escalate` 哪种后续处理，以及 `issues / sessions / entitlements / points` 这类影响范围提示，并直接给出可点击的 follow-up 按钮；同时快照顶部会额外突出 `Escalate First` 高优先级对象，并把产品、用户、原因、建议动作以及影响范围指标压缩成更短摘要，这些重点摘要也会同步进导出的 `summary` 文本
