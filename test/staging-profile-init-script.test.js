...
test("staging profile init writes a secret-free profile with launch-duty output paths", () => {
  ...
  try {
    ...
    const productionProofPreflightCommand = `npm.cmd run launch:production-proof-preflight -- --profile-file ${outputFile} --execution-pack-file ${productionProofExecutionPackFile}`;
...
test("staging profile init prints ordered next commands in plain output", () => {
  ...
  try {
    ...
    assert.match(result.stdout, /Production proof preflight: npm\.cmd run launch:production-proof-preflight -- --profile-file .*staging-profile\.json --execution-pack-file artifacts\/staging\/PILOT_ALPHA\/beta\/production-proof-execution-pack\.md/);
...
test("staging profile init prints secret env proof in plain output without secret values", () => {
  const tempDir = mkdtempSync(join(tmpdir(), "rsl-profile-init-secret-env-plain-"));
  try {
    const outputFile = join(tempDir, "staging-profile.json");
    const result = runProfileInitPlain([
      "--base-url",
