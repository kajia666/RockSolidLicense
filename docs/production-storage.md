...
- `RSL_POSTGRES_URL`、`RSL_REDIS_URL`：当前作为生产部署规划占位配置，健康检查会展示它们是否已配置，便于后续迁移时核对环境。
...
1. 先把 SQLite 直连 SQL 访问收敛成 repository / gateway 边界。
2. 再把运行时状态从 `sqlite` 切到真正的 `redis`。
