...
test("developer release package export bundles integration, versions, and notices inside scope", async () => {
  ...
  try {
    ...
    assert.equal(releasePackage.mainlineFollowUp.initialLaunchOpsReadiness.status, "not_started");
    assert.equal(releasePackage.mainlineFollowUp.initialLaunchOpsReadiness.gate.decision, "hold");
    assert.equal(releasePackage.mainlineFollowUp.initialLaunchOpsReadiness.gate.canEnterInitialLaunch, false);
    assert.ok(releasePackage.mainlineFollowUp.initialLaunchOpsReadiness.gate.blockers.some((item) => item.stage === "launch_receipt"));
    assert.equal(releasePackage.manifest.release.initialLaunchOpsReadiness.gate.decision, "hold");
    ...
    assert.ok(releasePackage.mainlineFollowUp.workspaceActions.some((item) => item.key === "ops"));
    ...
    assert.match(releaseSummaryDownload.body, /Initial Launch Ops Gate:/);
    ...
    const launchHandoffZipRecommended = launchWorkflow.workflowSummary.recommendedDownloads.find((item) => item.key === "launch_handoff_zip");
    assert.ok(launchHandoffZipRecommended);
    assert.equal(launchHandoffZipRecommended.source, "developer-launch-workflow");
    assert.equal(launchHandoffZipRecommended.format, "handoff-zip");
    assert.match(launchHandoffZipRecommended.href || "", /\/api\/developer\/launch-workflow\/download\?/);
    assert.match(launchHandoffZipRecommended.href || "", /format=handoff-zip/);
    const launchHandoffChecksumsRecommended = launchWorkflow.workflowSummary.recommendedDownloads.find((item) => item.key === "launch_handoff_checksums");
    assert.ok(launchHandoffChecksumsRecommended);
    assert.equal(launchHandoffChecksumsRecommended.source, "developer-launch-workflow");
    ...
    const integrationEnvRecommended = launchWorkflow.workflowSummary.recommendedDownloads.find((item) => item.key === "integration_env");
    assert.ok(integrationEnvRecommended);
    assert.equal(integrationEnvRecommended.source, "developer-integration-package");
    assert.equal(integrationEnvRecommended.format, "env");
    assert.match(integrationEnvRecommended.href || "", /\/api\/developer\/integration\/package\/download\?/);
    assert.match(integrationEnvRecommended.href || "", /productCode=RELPKG_ALPHA/);
    assert.match(integrationEnvRecommended.href || "", /channel=stable/);
    assert.match(integrationEnvRecommended.href || "", /format=env/);
    const integrationHostConfigRecommended = launchWorkflow.workflowSummary.recommendedDownloads.find((item) => item.key === "integration_host_config");
    assert.ok(integrationHostConfigRecommended);
    assert.equal(integrationHostConfigRecommended.source, "developer-integration-package");
    assert.equal(integrationHostConfigRecommended.format, "host-config");
    assert.match(integrationHostConfigRecommended.href || "", /\/api\/developer\/integration\/package\/download\?/);
    assert.match(integrationHostConfigRecommended.href || "", /format=host-config/);
    const integrationHostSkeletonRecommended = launchWorkflow.workflowSummary.recommendedDownloads.find((item) => item.key === "integration_host_skeleton");
    assert.ok(integrationHostSkeletonRecommended);
    assert.equal(integrationHostSkeletonRecommended.source, "developer-integration-package");
    assert.equal(integrationHostSkeletonRecommended.format, "host-skeleton");
    assert.match(integrationHostSkeletonRecommended.href || "", /\/api\/developer\/integration\/package\/download\?/);
    assert.match(integrationHostSkeletonRecommended.href || "", /format=host-skeleton/);
    ...
    const startupChecklistItem = launchWorkflow.workflowChecklist.items.find((item) => item.key === "startup_bootstrap");
    assert.equal(startupChecklistItem?.recommendedDownload?.source, "developer-integration-package");
    assert.equal(startupChecklistItem?.recommendedDownload?.format, "env");
    assert.match(startupChecklistItem?.recommendedDownload?.href || "", /\/api\/developer\/integration\/package\/download\?/);
    assert.match(startupChecklistItem?.recommendedDownload?.href || "", /productCode=RELPKG_ALPHA/);
    assert.match(startupChecklistItem?.recommendedDownload?.href || "", /channel=stable/);
    assert.match(startupChecklistItem?.recommendedDownload?.href || "", /format=env/);
    const tokenKeysChecklistItem = launchWorkflow.workflowChecklist.items.find((item) => item.key === "token_keys");
    assert.equal(tokenKeysChecklistItem?.recommendedDownload?.source, "developer-integration-package");
    assert.equal(tokenKeysChecklistItem?.recommendedDownload?.format, "host-config");
    assert.match(tokenKeysChecklistItem?.recommendedDownload?.href || "", /format=host-config/);
    ...
    const integrationFollowUpAction = launchWorkflow.workflowSummary.actionPlan.find((item) => item.key === "integration_follow_up");
    assert.equal(integrationFollowUpAction?.recommendedDownload?.source, "developer-integration-package");
    assert.equal(integrationFollowUpAction?.recommendedDownload?.format, "env");
    assert.match(integrationFollowUpAction?.recommendedDownload?.href || "", /productCode=RELPKG_ALPHA/);
    assert.match(integrationFollowUpAction?.recommendedDownload?.href || "", /channel=stable/);
    assert.match(integrationFollowUpAction?.recommendedDownload?.href || "", /format=env/);
    ...
    assert.ok(launchReview.reviewSummary.recommendedDownloads?.some((item) => item.key === "launch_mainline_checksums" && item.source === "developer-launch-mainline"));
      ...
      assert.match(launchReviewSummaryShortcut?.href || "", /channel=stable/);
      assert.match(launchReviewSummaryShortcut?.href || "", /source=launch-smoke/);
      assert.match(launchReviewSummaryShortcut?.href || "", /handoff=first-wave/);
      const launchReviewWorkflowSummaryShortcut = launchReview.reviewSummary.recommendedDownloads?.find((item) => item.key === "launch_summary");
      assert.equal(launchReviewWorkflowSummaryShortcut?.source, "developer-launch-workflow");
      assert.equal(launchReviewWorkflowSummaryShortcut?.format, "summary");
      assert.match(launchReviewWorkflowSummaryShortcut?.href || "", /\/api\/developer\/launch-workflow\/download\?/);
      assert.match(launchReviewWorkflowSummaryShortcut?.href || "", /productCode=RELPKG_ALPHA/);
      assert.match(launchReviewWorkflowSummaryShortcut?.href || "", /channel=stable/);
      assert.match(launchReviewWorkflowSummaryShortcut?.href || "", /source=launch-smoke/);
      assert.match(launchReviewWorkflowSummaryShortcut?.href || "", /handoff=first-wave/);
      const launchReviewWorkflowChecklistShortcut = launchReview.reviewSummary.recommendedDownloads?.find((item) => item.key === "launch_checklist");
      assert.equal(launchReviewWorkflowChecklistShortcut?.source, "developer-launch-workflow");
      assert.equal(launchReviewWorkflowChecklistShortcut?.format, "checklist");
      assert.match(launchReviewWorkflowChecklistShortcut?.href || "", /\/api\/developer\/launch-workflow\/download\?/);
      assert.match(launchReviewWorkflowChecklistShortcut?.href || "", /productCode=RELPKG_ALPHA/);
      assert.match(launchReviewWorkflowChecklistShortcut?.href || "", /channel=stable/);
      assert.match(launchReviewWorkflowChecklistShortcut?.href || "", /source=launch-smoke/);
      assert.match(launchReviewWorkflowChecklistShortcut?.href || "", /handoff=first-wave/);
      ...
      const launchReviewOpsControl = launchReview.reviewSummary.routeFocus?.controls?.find((item) => item.workspaceAction?.key === "ops");
      assert.match(launchReviewOpsControl?.workspaceAction?.href || "", /\/developer\/ops\?/);
      assert.match(launchReviewOpsControl?.workspaceAction?.href || "", /productCode=RELPKG_ALPHA/);
      assert.match(launchReviewOpsControl?.workspaceAction?.href || "", /channel=stable/);
      assert.match(launchReviewOpsControl?.workspaceAction?.href || "", /source=launch-smoke/);
      assert.match(launchReviewOpsControl?.workspaceAction?.href || "", /handoff=first-wave/);
      ...
      assert.match(launchReview.summaryText, /Staging Archive Launch Runway: closeout_reload -> full_test_window -> production_signoff -> launch_day_watch/);
      assert.match(launchReview.summaryText, /Staging Archive Runway Current Gate: ready_for_closeout_reload/);
      assert.match(launchReview.summaryText, /Staging Archive Launch-Day Watch Entry: enter_after_production_signoff/);
      ...
      assert.match(launchReviewSummaryDownload.body, /Staging Archive Launch Runway: closeout_reload -> full_test_window -> production_signoff -> launch_day_watch/);
      assert.match(launchReviewSummaryDownload.body, /Staging Archive Runway Current Gate: ready_for_closeout_reload/);
      assert.match(launchReviewSummaryDownload.body, /Staging Archive Launch-Day Watch Entry: enter_after_production_signoff/);
      ...
      assert.equal(launchMainline.mainlineSummary.initialLaunchOpsReadiness?.status, "not_started");
      assert.deepEqual(
        launchMainline.mainlineSummary.initialLaunchOpsGate,
        launchMainline.opsSnapshot.summary.initialLaunchOpsReadiness.gate
      );
      assert.equal(launchMainline.mainlineSummary.initialLaunchOpsGate?.decision, "hold");
      assert.equal(launchMainline.mainlineSummary.initialLaunchOpsGate?.canEnterInitialLaunch, false);
      ...
      assert.ok(launchMainline.mainlineSummary.stages.some((item) => item.key === "initial_launch_ops" && item.gate?.status === "hold"));
      ...
      const mainlineOpsPrimaryDownload = launchMainline.mainlineSummary.recommendedDownloads.find((item) => item.key === "route_review_primary");
      assert.ok(mainlineOpsPrimaryDownload);
      assert.equal(mainlineOpsPrimaryDownload.source, "developer-ops");
      assert.match(mainlineOpsPrimaryDownload.href || "", /\/api\/developer\/ops\/export\/download\?/);
      assert.match(mainlineOpsPrimaryDownload.href || "", /productCode=RELPKG_ALPHA/);
      assert.match(mainlineOpsPrimaryDownload.href || "", /channel=stable/);
      assert.match(mainlineOpsPrimaryDownload.href || "", /eventType=session\.login/);
      assert.match(mainlineOpsPrimaryDownload.href || "", /actorType=account/);
      assert.match(mainlineOpsPrimaryDownload.href || "", /format=route-review-primary/);
      const mainlineOpsRemainingDownload = launchMainline.mainlineSummary.recommendedDownloads.find((item) => item.key === "route_review_remaining");
      assert.ok(mainlineOpsRemainingDownload);
      assert.equal(mainlineOpsRemainingDownload.source, "developer-ops");
      assert.match(mainlineOpsRemainingDownload.href || "", /\/api\/developer\/ops\/export\/download\?/);
      assert.match(mainlineOpsRemainingDownload.href || "", /productCode=RELPKG_ALPHA/);
      assert.match(mainlineOpsRemainingDownload.href || "", /channel=stable/);
      assert.match(mainlineOpsRemainingDownload.href || "", /eventType=session\.login/);
      assert.match(mainlineOpsRemainingDownload.href || "", /actorType=account/);
      assert.match(mainlineOpsRemainingDownload.href || "", /format=route-review-remaining/);
      ...
      const mainlineLaunchRunway = launchMainline.mainlineSummary.launchRunway;
      assert.ok(mainlineLaunchRunway);
      assert.equal(mainlineLaunchRunway.source, "developer-ops-launch-duty-action-order");
      ...
      assert.deepEqual(
        mainlineLaunchRunway.copyActions.map((item) => [item.key, item.kind]),
        [
          ["closeout_reload", "command"],
          ["full_test_window", "command"],
          ["production_signoff_packet", "artifact_path"]
        ]
      );
      assert.match(
        mainlineLaunchRunway.copyActions.find((item) => item.key === "closeout_reload")?.value || "",
        /npm\.cmd run staging:rehearsal -- --profile-file docs\/staging-rehearsal-profile\.example\.json --product-code RELPKG_ALPHA --channel stable --closeout-input-file artifacts\/staging\/RELPKG_ALPHA\/stable\/filled-closeout-input\.json/
      );
      assert.equal(
        mainlineLaunchRunway.copyActions.find((item) => item.key === "full_test_window")?.value,
        "npm.cmd test"
      );
      assert.equal(
      ...
      assert.deepEqual({
        ...
        label: launchMainline.mainlineSummary.launchDayWatchPanel?.label,
        ...
        primaryWorkspaceAction: launchMainline.mainlineSummary.launchDayWatchPanel?.primaryWorkspaceAction
          ? {
              ...
              autofocus: launchMainline.mainlineSummary.launchDayWatchPanel.primaryWorkspaceAction.autofocus,
              label: launchMainline.mainlineSummary.launchDayWatchPanel.primaryWorkspaceAction.label
            }
          : null,
        primaryDownload: launchMainline.mainlineSummary.launchDayWatchPanel?.primaryDownload
          ? {
              key: launchMainline.mainlineSummary.launchDayWatchPanel.primaryDownload.key,
              format: launchMainline.mainlineSummary.launchDayWatchPanel.primaryDownload.format
            }
          : null,
        secondaryDownload: launchMainline.mainlineSummary.launchDayWatchPanel?.secondaryDownload
          ? {
              key: launchMainline.mainlineSummary.launchDayWatchPanel.secondaryDownload.key,
        ...
        label: "Launch-Day Watch Blocked",
      ...
      assert.ok(launchMainline.mainlineSummary.launchDayWatchPanel?.controls?.some((item) =>
        item?.label === "Record Remaining Runway Evidence"
      ...
      assert.match(operationsHandoffDownload.body, /Primary Lifecycle Download: .*href=.*\/api\/developer\/launch-mainline\/download\?.*format=operations-handoff/i);
      assert.match(operationsHandoffDownload.body, /Lifecycle Recommended Downloads:.*href=.*\/api\/developer\/launch-mainline\/download\?.*format=stabilization-handoff/i);
      ...
      assert.match(postLaunchSweepHandoffDownload.body, /Lifecycle Recommended Downloads:.*href=.*\/api\/developer\/launch-mainline\/download\?.*format=post-launch-sweep-handoff/i);
      ...
      assert.match(closeoutHandoffDownload.body, /Lifecycle Recommended Downloads:.*href=.*\/api\/developer\/launch-mainline\/download\?.*format=closeout-handoff/i);
      ...
      assert.match(stabilizationHandoffDownload.body, /Lifecycle Recommended Downloads:.*href=.*\/api\/developer\/launch-mainline\/download\?.*format=stabilization-handoff/i);
      ...
      assert.match(postLaunchHandoffIndexDownload.body, /Primary Lifecycle Download: .*href=.*\/api\/developer\/launch-mainline\/download\?.*format=operations-handoff/i);
...
test("developer launch runway next evidence action advances after each evidence receipt", async () => {
  ...
  try {
    ...
    assert.deepEqual({
      ...
      label: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.label,
      summary: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.summary,
      launchDayWatchReady: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.launchDayWatchReady,
      watchEntry: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.watchEntry,
      pendingEvidenceCount: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.pendingEvidenceCount,
      ...
      primaryWorkspaceAction: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.primaryWorkspaceAction
        ? {
            ...
            autofocus: sweepResult.launchMainline.mainlineSummary.launchDayWatchPanel.primaryWorkspaceAction.autofocus,
            label: sweepResult.launchMainline.mainlineSummary.launchDayWatchPanel.primaryWorkspaceAction.label
          }
        : null,
      primaryDownload: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.primaryDownload
        ? {
            key: sweepResult.launchMainline.mainlineSummary.launchDayWatchPanel.primaryDownload.key,
            format: sweepResult.launchMainline.mainlineSummary.launchDayWatchPanel.primaryDownload.format
          }
        : null,
      secondaryDownload: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.secondaryDownload
        ? {
            key: sweepResult.launchMainline.mainlineSummary.launchDayWatchPanel.secondaryDownload.key,
            format: sweepResult.launchMainline.mainlineSummary.launchDayWatchPanel.secondaryDownload.format
          }
        : null,
      firstWaveCheckpoints: sweepResult.launchMainline?.mainlineSummary?.launchDayWatchPanel?.firstWaveCheckpoints
    }, {
      status: "ready_for_launch_day_watch",
      label: "Launch-Day Watch Ready",
      summary: "Runway evidence is complete. Enter launch-day watch after production sign-off and keep the first-wave checkpoints close.",
      launchDayWatchReady: true,
      watchEntry: "enter_after_production_signoff",
      pendingEvidenceCount: 0,
      pendingEvidenceOperationCount: 0,
      nextActionKey: null,
...
test("developer license quickstart bootstrap can create starter launch assets in one pass", async () => {
  ...
  try {
    ...
    assert.match(smokeKit.summaryText || "", /Staging Archive Launch Runway: closeout_reload -> full_test_window -> production_signoff -> launch_day_watch/);
    assert.match(smokeKit.summaryText || "", /Staging Archive Runway Current Gate: ready_for_closeout_reload/);
    assert.match(smokeKit.summaryText || "", /Staging Archive Launch-Day Watch Entry: enter_after_production_signoff/);
    ...
    assert.match(smokeKitSummaryDownload.body, /Staging Archive Launch Runway: closeout_reload -> full_test_window -> production_signoff -> launch_day_watch/);
    assert.match(smokeKitSummaryDownload.body, /Staging Archive Runway Current Gate: ready_for_closeout_reload/);
    assert.match(smokeKitSummaryDownload.body, /Staging Archive Launch-Day Watch Entry: enter_after_production_signoff/);
...
test("developer launch mainline action can create first launch batches and return refreshed mainline", async () => {
  ...
  try {
    ...
    assert.match(firstLaunchHandoffDownloadResponse.body, /Workspace actions:/);
    assert.match(firstLaunchHandoffDownloadResponse.body, /Recommended downloads:/);
    assert.match(firstLaunchHandoffDownloadResponse.body, /Recommended downloads:.*href=.*\/api\/developer\/launch-mainline\/download\?.*format=first-launch-handoff/i);
    assert.match(firstLaunchHandoffDownloadResponse.body, /Duty Chain:/);
    assert.match(firstLaunchHandoffDownloadResponse.body, /Post-Launch Lifecycle:/);
    assert.match(firstLaunchHandoffDownloadResponse.body, /Lifecycle Recommended Downloads:.*Launch mainline operations handoff/i);
...
test("developer first-wave recommendations summarize launch inventory, card issuance, and ops actions", async () => {
  ...
  try {
    ...
    assert.equal(mainlineFirstWaveBridgeCard.title, "First-Wave Readiness Bridge");
    assert.ok(mainlineFirstWaveBridgeCard.tags.some((item) => item.label === "status" && item.value === "ready_for_first_wave_handoff"));
    assert.ok(mainlineFirstWaveBridgeCard.tags.some((item) => item.label === "gate" && item.value === "first_round_ops"));
    ...
    assert.ok(mainlineFirstWaveBridgeCard.controls.some((item) =>
      item.kind === "download"
      && item.label === "Download First-Wave JSON"
      && item.recommendedDownload?.format === "json"
      && /format=json/.test(item.recommendedDownload?.href || "")
    ));
    assert.ok(mainlineFirstWaveBridgeCard.controls.some((item) =>
      item.kind === "download"
      && item.label === "Download First-Wave Checksums"
      && item.recommendedDownload?.format === "checksums"
      && /format=checksums/.test(item.recommendedDownload?.href || "")
    ));
    assert.ok(mainlineFirstWaveBridgeCard.controls.some((item) =>
      item.kind === "confirm"
...
test("developer ops export bundles scoped data and downloadable assets", async () => {
  ...
  try {
    ...
    const opsRoutePrimaryDownload = exportSnapshot.routeReview?.downloads?.primary;
    assert.equal(opsRoutePrimaryDownload?.source, "developer-ops");
    assert.equal(opsRoutePrimaryDownload?.format, "route-review-primary");
    ...
    const initialLaunchGoNoGo = launchReceiptSnapshot.summary.initialLaunchOpsReadiness.goNoGo;
    assert.ok(initialLaunchGoNoGo);
    assert.equal(initialLaunchGoNoGo.decision, "no_go");
    assert.equal(initialLaunchGoNoGo.label, "NO-GO");
    assert.equal(initialLaunchGoNoGo.canEnterInitialLaunch, false);
    assert.equal(initialLaunchGoNoGo.status, "review");
    assert.equal(initialLaunchGoNoGo.primaryBlockerStage, "production_evidence");
    ...
    assert.ok(initialLaunchContract);
    assert.equal(initialLaunchContract.version, "initial-launch-ops/v1");
    assert.equal(initialLaunchContract.projectCode, "EXPORT_ALPHA");
    assert.equal(initialLaunchContract.channel, "stable");
    assert.equal(initialLaunchContract.decision, "no_go");
    assert.equal(initialLaunchContract.label, "NO-GO");
    assert.equal(initialLaunchContract.canEnterInitialLaunch, false);
    assert.equal(initialLaunchContract.latestLaunchReceipt.operation, "record_post_launch_ops_sweep");
    assert.equal(initialLaunchContract.latestLaunchReceipt.handoffFileName, latestLaunchReceipt.handoffFileName);
    ...
    assert.equal(initialLaunchContract.files.initialLaunchOpsReadiness, "initial-launch-ops-readiness.txt");
    ...
    const initialLaunchOperatorNextAction = launchReceiptSnapshot.summary.initialLaunchOpsReadiness.operatorNextAction;
    assert.ok(initialLaunchOperatorNextAction);
    assert.equal(initialLaunchOperatorNextAction.kind, "launch_mainline_follow_up");
    assert.equal(initialLaunchOperatorNextAction.source, "initial_launch_operator");
    assert.equal(initialLaunchOperatorNextAction.decision, "no_go");
    assert.equal(initialLaunchOperatorNextAction.stage, "production_evidence");
    assert.equal(initialLaunchOperatorNextAction.operation, latestLaunchReceipt.productionEvidenceNextOperation);
    assert.equal(initialLaunchOperatorNextAction.actionKey, latestLaunchReceipt.productionEvidenceNextActionKey);
    assert.equal(initialLaunchOperatorNextAction.workspaceAction.key, "launch-mainline");
    assert.equal(initialLaunchOperatorNextAction.workspaceAction.autofocus, "summary");
    assert.equal(initialLaunchOperatorNextAction.workspaceAction.params.operation, latestLaunchReceipt.productionEvidenceNextOperation);
    assert.match(initialLaunchOperatorNextAction.workspaceAction.href, /\/developer\/launch-mainline\?/);
    assert.equal(initialLaunchOperatorNextAction.download.fileName, "developer-ops-launch-receipt-next-follow-up.txt");
    assert.equal(initialLaunchOperatorNextAction.download.format, "launch-receipt-next-follow-up");
    assert.equal(initialLaunchOperatorNextAction.traceability.handoffIndex, "handoff-index.txt");
    const initialLaunchOperatorActionManifest = launchReceiptSnapshot.summary.initialLaunchOpsReadiness.operatorActionManifest;
    assert.ok(initialLaunchOperatorActionManifest);
    assert.equal(initialLaunchOperatorActionManifest.version, "initial-launch-operator-actions/v1");
    assert.equal(initialLaunchOperatorActionManifest.generatedFor, "initial_launch_operator");
    assert.equal(initialLaunchOperatorActionManifest.projectCode, "EXPORT_ALPHA");
    assert.equal(initialLaunchOperatorActionManifest.channel, "stable");
    assert.equal(initialLaunchOperatorActionManifest.decision, "no_go");
    assert.equal(initialLaunchOperatorActionManifest.primaryActionKey, "launch_mainline_follow_up");
    assert.equal(initialLaunchOperatorActionManifest.actionCount, 1);
    assert.equal(initialLaunchOperatorActionManifest.actions[0].operation, latestLaunchReceipt.productionEvidenceNextOperation);
    assert.equal(initialLaunchOperatorActionManifest.actions[0].workspaceAction.key, "launch-mainline");
    ...
    assert.match(launchReceiptSnapshot.summaryText, /- gate: HOLD \(canEnterInitialLaunch=false\)/);
    assert.match(launchReceiptSnapshot.summaryText, /- blockers: [1-9]/);
    ...
    assert.match(launchMainlinePostLaunchIndexDownload.body, /Staging Archive Launch Runway: closeout_reload -> full_test_window -> production_signoff -> launch_day_watch/);
    assert.match(launchMainlinePostLaunchIndexDownload.body, /Staging Archive Runway Current Gate: ready_for_closeout_reload/);
    assert.match(launchMainlinePostLaunchIndexDownload.body, /Staging Archive Launch-Day Watch Entry: enter_after_production_signoff/);
    ...
    assert.match(
      launchMainlineTraceabilityZipText,
      /ops-handoff-index: ops\/handoff-index\.txt.*format=handoff-index.*href=.*\/api\/developer\/ops\/export\/download\?.*format=handoff-index/
    );
    assert.match(launchMainlineTraceabilityZipText, /launch-mainline-post-launch-handoff-index/);
    assert.match(
      launchMainlineTraceabilityZipText,
    ...
    assert.equal(
      launchMainlineTraceability.postLaunchHandoffTraceability.initialLaunchOperatorNextAction.kind,
      "launch_mainline_follow_up"
    );
    assert.equal(
      launchMainlineTraceability.postLaunchHandoffTraceability.initialLaunchOperatorNextAction.operation,
      latestLaunchReceipt.productionEvidenceNextOperation
    );
    assert.equal(
      launchMainlineTraceability.postLaunchHandoffTraceability.initialLaunchOperatorNextAction.download.fileName,
      "developer-ops-launch-receipt-next-follow-up.txt"
    );
    assert.equal(
      launchMainlineTraceability.postLaunchHandoffTraceability.initialLaunchOperatorActionManifest.version,
      "initial-launch-operator-actions/v1"
    );
    assert.equal(
      launchMainlineTraceability.postLaunchHandoffTraceability.initialLaunchOperatorActionManifest.primaryActionKey,
      "launch_mainline_follow_up"
    );
    assert.equal(
      launchMainlineTraceability.postLaunchHandoffTraceability.initialLaunchOperatorActionManifest.actions[0].operation,
      latestLaunchReceipt.productionEvidenceNextOperation
    );
    ...
    assert.match(launchMainlineTraceabilitySummaryDownload.body, /Initial Launch Operator Next Action:/);
    assert.match(
      launchMainlineTraceabilitySummaryDownload.body,
      new RegExp(`Action: launch_mainline_follow_up \\| stage=production_evidence \\| operation=${latestLaunchReceipt.productionEvidenceNextOperation}`)
    );
...
test("developer launch mainline page is served from the dedicated route", async () => {
  ...
  try {
    ...
    assert.match(html, /Initial Launch Operator Actions/);
    ...
    assert.match(html, /data-mainline-operator-action-index/);
...
test("developer operations page is served from the dedicated route", async () => {
  ...
  try {
    ...
    assert.match(html, /Initial Launch Stabilization Handoff/);
    assert.match(html, /stabilization-handoff/);
    ...
    assert.match(html, /buildStabilizationHandoffActionResult/);
    assert.match(html, /renderLastStabilizationHandoffResult/);
    assert.match(html, /Last Stabilization Handoff Action/);
    assert.match(html, /stabilization-handoff-confirmation/);
    assert.match(html, /Stabilization handoff action captured/);
    assert.match(html, /Launch Receipt Follow-ups/);
...
test("developer release package mainline follow-up carries launch authorization actions", async () => {
  ...
  try {
    ...
    assert.ok(
      secondReleasePackage.mainlineFollowUp.actionPlan.some((item) =>
        item.key === "initial_launch_ops_readiness" && item.workspaceAction?.key === "ops"
      )
    );
    assert.ok(
    ...
    assert.doesNotMatch(secondReleasePackage.summaryText, /\| setup=Run First Batch Setup@recommended:first_batch_setup/);
  } finally {
    await app.close();
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
