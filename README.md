...
- `read_side_preview`：`products / policies / cards / entitlements / accounts / devices` 六组主数据读侧走 PostgreSQL；`devices` 写侧和 `sessions` 仍在 SQLite
- `core_write_preview`：如果 adapter 额外支持事务接口 `withTransaction(...)`，则 `products / policies / cards / entitlements / accounts` 五组核心主数据写侧也会走 PostgreSQL；`devices` 会进入 `postgres_partial`，先覆盖绑定释放与自助解绑日志链路，`sessions` 仍保留在 SQLite
