...
- review focus account, session, and device detail lists directly from the snapshot overview and click them back into the active filters
...
- the ops snapshot preview and JSON payload now include an `overview` block with a headline, attention counters, highlight bullets, top audit-event counts, common reasons, and focus account/session/device detail lists for faster scoped troubleshooting
