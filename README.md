...
- `core_write_preview`：如果 adapter 额外支持事务接口 `withTransaction(...)`，则 `products / policies / cards / entitlements / accounts` 五组核心主数据写侧也会走 PostgreSQL；`devices / sessions` 写侧仍保留在 SQLite
