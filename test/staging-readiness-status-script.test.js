...
test("staging readiness status plain output prints production signoff artifact and example hints", () => {
  ...
  try {
    ...
    assert.match(result.stdout, /Production signoff handoff: ready_for_production_signoff_evidence/);
    assert.match(result.stdout, /Production signoff current action: backfill_production_signoff/);
    assert.match(result.stdout, /Production signoff target: production_signoff_condition\/staging_artifacts_archived/);
    assert.match(result.stdout, /Production signoff command: npm\.cmd run staging:signoff:backfill -- --input-file .*filled-closeout-input\.json --condition-key staging_artifacts_archived --value-json <redacted-json> --actions-file .*readiness-action-queue\.md/);
    assert.match(result.stdout, /Production signoff example: npm\.cmd run staging:signoff:backfill -- --input-file .*filled-closeout-input\.json --condition-key staging_artifacts_archived --value-json '\{"result":"confirmed","summary":"<redacted operator summary>"\}' --artifact-path artifacts\/staging\/<productCode>\/<channel>\/staging-artifacts-archive\.txt --actions-file .*readiness-action-queue\.md/);
    assert.match(result.stdout, /Production signoff artifact: artifacts\/staging\/<productCode>\/<channel>\/staging-artifacts-archive\.txt/);
    assert.match(result.stdout, /Production signoff status refresh: npm\.cmd run staging:readiness:status -- --input-file .*filled-closeout-input\.json --actions-file .*readiness-action-queue\.md/);
    assert.match(result.stdout, /Production signoff rehearsal reload: npm\.cmd run staging:rehearsal -- --closeout-input-file .*filled-closeout-input\.json/);
    assert.match(result.stdout, /Production signoff next action: Run command with real redacted evidence, then statusCommand to continue production sign-off\./);
...
test("staging readiness status reports launch-day watch readiness after all local gates are clear", () => {
  ...
  try {
    ...
    const expectedLaunchDutyRecordIndex = "artifacts/staging/PILOT_ALPHA/stable/launch-duty-record-index.json";
    const watchRecordCommand = output.launchDutyNextRun.postArchiveEvidenceActions[0].recordCommand;
    const firstWaveRecordCommand = output.launchDutyNextRun.postArchiveEvidenceActions[1].recordCommand;
    assert.match(watchRecordCommand, /--record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json/);
    assert.match(firstWaveRecordCommand, /--record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json/);
    assert.doesNotMatch(firstWaveRecordCommand, /--source-record .*<.*artifact-path>/);
    assert.equal(output.launchDutyNextRun.recordIndexFile, expectedLaunchDutyRecordIndex);
    assert.equal(output.launchDutyWatchHandoff.recordIndexFile, expectedLaunchDutyRecordIndex);
    assert.equal(output.launchDutyWatchHandoff.watchSummary.recordIndexFile, expectedLaunchDutyRecordIndex);
    assert.equal(output.launchDutyWatchHandoff.firstWaveCloseout.recordIndexFile, expectedLaunchDutyRecordIndex);
    assert.equal(output.operatorNextCommands[1].recordIndexFile, expectedLaunchDutyRecordIndex);
    assert.equal(output.operatorNextCommands[2].recordIndexFile, expectedLaunchDutyRecordIndex);
    assert.deepEqual(output.launchDutyNextRun, {
      status: "ready_for_launch_day_watch",
      currentActionKey: "archive_production_signoff",
      reloadCommand: `npm.cmd run staging:rehearsal -- --closeout-input-file ${inputFile}`,
      recordIndexFile: expectedLaunchDutyRecordIndex,
      actionKeys: [
      ...
      postArchiveEvidenceActions: [
        {
          ...
          recordCommand: `npm.cmd run staging:launch-duty:record -- --closeout-input-file ${inputFile} --key launch_day_watch_summary --artifact-path artifacts/staging/PILOT_ALPHA/stable/launch-day-watch-summary.md --value-json <redacted-json> --receipt-id <record_cutover_walkthrough-receipt-id> --receipt-id <record_launch_day_readiness_review-receipt-id> --record-index-file ${expectedLaunchDutyRecordIndex} --actions-file ${actionsFile}`,
          exampleCommand: `npm.cmd run staging:launch-duty:record -- --closeout-input-file ${inputFile} --key launch_day_watch_summary --artifact-path artifacts/staging/PILOT_ALPHA/stable/launch-day-watch-summary.md --value-json '{"result":"recorded","watchWindow":"T-30m through T+2h","summary":"<redacted launch-day watch summary>"}' --receipt-id <record_cutover_walkthrough-receipt-id> --receipt-id <record_launch_day_readiness_review-receipt-id> --record-index-file ${expectedLaunchDutyRecordIndex} --actions-file ${actionsFile}`,
        ...
        },
        {
          ...
          recordCommand: `npm.cmd run staging:launch-duty:record -- --closeout-input-file ${inputFile} --key first_wave_closeout --artifact-path artifacts/staging/PILOT_ALPHA/stable/first-wave-closeout.md --value-json <redacted-json> --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file ${expectedLaunchDutyRecordIndex} --actions-file ${actionsFile}`,
          exampleCommand: `npm.cmd run staging:launch-duty:record -- --closeout-input-file ${inputFile} --key first_wave_closeout --artifact-path artifacts/staging/PILOT_ALPHA/stable/first-wave-closeout.md --value-json '{"result":"closed","unresolvedIncidents":[],"summary":"<redacted first-wave closeout summary>"}' --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file ${expectedLaunchDutyRecordIndex} --actions-file ${actionsFile}`,
    ...
    assert.deepEqual(output.operatorNextCommands, [
      ...
      },
      {
        ...
        phase: "launch_day_watch",
        actionKey: "record_launch_day_watch_summary",
        command: `npm.cmd run staging:launch-duty:record -- --closeout-input-file ${inputFile} --key launch_day_watch_summary --artifact-path artifacts/staging/PILOT_ALPHA/stable/launch-day-watch-summary.md --value-json <redacted-json> --receipt-id <record_cutover_walkthrough-receipt-id> --receipt-id <record_launch_day_readiness_review-receipt-id> --record-index-file ${expectedLaunchDutyRecordIndex} --actions-file ${actionsFile}`,
        statusCommand: `npm.cmd run staging:readiness:status -- --input-file ${inputFile} --actions-file ${actionsFile}`,
      ...
      },
      {
        ...
        phase: "first_wave_closeout",
        actionKey: "close_first_wave",
        command: `npm.cmd run staging:launch-duty:record -- --closeout-input-file ${inputFile} --key first_wave_closeout --artifact-path artifacts/staging/PILOT_ALPHA/stable/first-wave-closeout.md --value-json <redacted-json> --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file ${expectedLaunchDutyRecordIndex} --actions-file ${actionsFile}`,
        statusCommand: `npm.cmd run staging:readiness:status -- --input-file ${inputFile} --actions-file ${actionsFile}`,
    ...
    assert.match(markdown, /Launch-duty record index: `artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json`/);
    ...
    assert.match(markdown, /Handoff first-wave closeout command: `npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key first_wave_closeout --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/first-wave-closeout\.md --value-json <redacted-json> --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md`/);
    ...
    assert.match(markdown, /blocked_after_rehearsal_reload: record_launch_day_watch_summary -> `npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key launch_day_watch_summary --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/launch-day-watch-summary\.md --value-json <redacted-json> --receipt-id <record_cutover_walkthrough-receipt-id> --receipt-id <record_launch_day_readiness_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md`/);
    assert.match(markdown, /blocked_after_launch_day_watch_summary: close_first_wave -> `npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key first_wave_closeout --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/first-wave-closeout\.md --value-json <redacted-json> --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md`/);
...
test("staging readiness status reports stabilization handoff when launch-duty record index is complete", () => {
  ...
  try {
    ...
    assert.deepEqual(output.stableOperationsRolloutWideningReadinessBridge, {
    ...
    assert.match(markdown, /First-result handoff: `first-operating-result-handoff-execution\.txt` `first-operating-result-handoff-execution` -> `https:\/\/staging\.example\.com\/api\/developer\/launch-mainline\/download\?productCode=PILOT_ALPHA&channel=stable&reviewMode=matched&format=first-operating-result-handoff-execution`/);
    ...
    assert.match(markdown, /First-result review: `first-operating-result-review-execution\.txt` `first-operating-result-review-execution` -> `https:\/\/staging\.example\.com\/api\/developer\/launch-mainline\/download\?productCode=PILOT_ALPHA&channel=stable&reviewMode=matched&format=first-operating-result-review-execution`/);
    assert.match(markdown, /## Stable Operations Rollout Widening Readiness Bridge/);
    assert.match(markdown, /Rollout readiness bridge: `ready_for_rollout_widening_rehearsal_readback`/);
    assert.match(markdown, /Rollout expected rehearsal bridge: `ready_for_rollout_widening_entrypoints` current `review_rollout_widening_decision` next `handoff_first_operating_result`/);
    assert.match(markdown, /Rollout widening decision: `rollout-widening-decision-execution\.txt` `rollout-widening-decision-execution` -> `https:\/\/staging\.example\.com\/api\/developer\/launch-mainline\/download\?productCode=PILOT_ALPHA&channel=stable&reviewMode=matched&format=rollout-widening-decision-execution`/);
    assert.match(markdown, /Rollout next decision: `next-rollout-widening-decision-execution\.txt` `next-rollout-widening-decision-execution` -> `https:\/\/staging\.example\.com\/api\/developer\/launch-mainline\/download\?productCode=PILOT_ALPHA&channel=stable&reviewMode=matched&format=next-rollout-widening-decision-execution`/);
    assert.match(markdown, /Rollout monitoring: `widened-rollout-monitoring-execution\.txt` `widened-rollout-monitoring-execution` -> `https:\/\/staging\.example\.com\/api\/developer\/launch-mainline\/download\?productCode=PILOT_ALPHA&channel=stable&reviewMode=matched&format=widened-rollout-monitoring-execution`/);
    ...
    assert.match(plain.stdout, /Stable rollout readiness bridge: ready_for_rollout_widening_rehearsal_readback \(current=reload_rehearsal_for_stabilization_handoff, next=review_rollout_widening_decision\)/);
    assert.match(plain.stdout, /Stable rollout expected rehearsal: ready_for_rollout_widening_entrypoints \(current=review_rollout_widening_decision, next=handoff_first_operating_result\)/);
...
test("staging readiness status prints launch-duty next run in plain output", () => {
  ...
  try {
    ...
    assert.match(result.stdout, /Launch duty record index: artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json/);
    ...
    assert.match(result.stdout, /Launch duty watch record index: artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json/);
    ...
    assert.match(result.stdout, /Launch duty watch summary command: npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key launch_day_watch_summary --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/launch-day-watch-summary\.md --value-json <redacted-json> --receipt-id <record_cutover_walkthrough-receipt-id> --receipt-id <record_launch_day_readiness_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md/);
    ...
    assert.match(result.stdout, /Launch duty first-wave closeout command: npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key first_wave_closeout --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/first-wave-closeout\.md --value-json <redacted-json> --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md/);
    ...
    assert.match(result.stdout, /Operator next blocked_after_rehearsal_reload: record_launch_day_watch_summary -> npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key launch_day_watch_summary --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/launch-day-watch-summary\.md --value-json <redacted-json> --receipt-id <record_cutover_walkthrough-receipt-id> --receipt-id <record_launch_day_readiness_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md/);
    ...
    assert.match(result.stdout, /Operator next blocked_after_rehearsal_reload record index: artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json/);
    ...
    assert.match(result.stdout, /Operator next blocked_after_launch_day_watch_summary: close_first_wave -> npm\.cmd run staging:launch-duty:record -- --closeout-input-file .*filled-closeout-input\.json --key first_wave_closeout --artifact-path artifacts\/staging\/PILOT_ALPHA\/stable\/first-wave-closeout\.md --value-json <redacted-json> --receipt-id <record_launch_closeout_review-receipt-id> --record-index-file artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json --actions-file .*readiness-action-queue\.md/);
    assert.match(result.stdout, /Operator next blocked_after_launch_day_watch_summary receipts: record_launch_closeout_review/);
    assert.match(result.stdout, /Operator next blocked_after_launch_day_watch_summary record index: artifacts\/staging\/PILOT_ALPHA\/stable\/launch-duty-record-index\.json/);
