...
The preceding `staging:preflight` output also includes `liveWriteSmokeHandoff`. That handoff keeps the exact `launch:smoke:staging` command, explicit closeout/action file paths, required smoke secret env names, expected post-smoke closeout backfill keys, readiness refresh, and the operator-confirmed `launch_smoke_staging` live-write gate together before the write command is run. After `launch:smoke:staging` succeeds, continue with its emitted `handoff.closeoutBackfill` queue to backfill `live_write_smoke_result`, `launch_smoke_handoff`, `launch_mainline_evidence_receipts`, and `receipt_visibility_review`. The same successful Launch Smoke result now also emits `handoff.postSmokeCloseoutHandoff`, which keeps the current closeout command, closeout/action files, readiness refresh command, remaining evidence keys, and blocked production sign-off gate together on the first screen. After those backfills are written, rerun `staging:readiness:status`; its `postSmokeReadinessBridge` reports the post-smoke closeout count, closeout readiness count, current full-test or sign-off command, `full_test_window_passed` backfill command, readiness refresh, and production sign-off gate in one readback. After the full-test result is backfilled with `staging:signoff:backfill --condition-key full_test_window_passed`, the script emits `postFullTestSignoffBridge`, which keeps the readiness refresh, next production sign-off backfill, launch-day watch blockers, and rehearsal reload together before the operator continues sign-off evidence. After the final receipt visibility lane is backfilled, `postReceiptVisibilityLaunchDayBridge` keeps the readiness refresh, rehearsal reload, production sign-off packet, launch-duty record index, `launch_day_watch_summary` command, and first-wave closeout artifact together for the launch-day watch handoff. After `launch_day_watch_summary` is recorded, `postLaunchDayWatchFirstWaveBridge` keeps the remaining first-wave record queue, predicted source-record artifact paths for `first_wave_closeout`, readiness refresh, rehearsal reload, and stable-operations handoff prerequisites together. After `first_wave_closeout` completes the 6/6 launch-duty record index, `postFirstWaveCloseoutStableOpsBridge` keeps the readiness refresh command, rehearsal reload command, expected readiness/rehearsal readbacks, launch-duty record index, and first-wave closeout artifact together before stable-operations handoff. The same stable tail now also has explicit readback bridges in both `staging:readiness:status` (`stableOperationsReadbackBridge`: refresh/reload current command plus expected rehearsal readback, `stableOperationsFirstResultReadinessBridge`: first operating-result direct files beside the readiness refresh, and `stableOperationsRolloutWideningReadinessBridge`: rollout widening, next decision, and widened monitoring direct files) and `staging:rehearsal` (`stableOperationsReadbackBridge`: stable handoff readiness/readback confirmation), so launch duty can verify the handoff transition from either entrypoint without reconstructing fields from deeper packets. After rehearsal confirms that readback, `staging:rehearsal` also emits `stableOperationsFirstDutyBridge`, which points directly to the steady-state handoff brief and the steady-state duty receipt review execution download for the same product/channel, keeping the first stable-operations duty files beside the launch-duty record index and first-wave closeout artifact. It also emits `stableOperationsFirstResultBridge` and `stableOperationsRolloutWideningBridge`, which keep Launch Mainline direct execution downloads for the first operating result, rollout widening decision, next rollout decision, and widened rollout monitoring together with the Ops overview-status download for the same lane. The `launch:route-map-gate --dry-run --json` operator queue mirrors this stable tail with `verify_stable_operations_first_result_and_rollout`, so the route-map dry run now shows the seven post-handoff proof downloads instead of stopping at the stable-operations handoff row.
